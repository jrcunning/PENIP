...
# Get coral data for selected sites
```{r}
...
# Summarize coral composition and aggregate taxa
```{r}
...
counts_long_ag <- counts_long %>%
  mutate(taxon = case_when(taxon %in% c("PPOR", "PFUR", "PDIV") ~ "PBRA",
                           ...
                           taxon %in% c("AFRA", "AGAR", "AAGA", "AHUM", "ALAM") ~ "AGAR",
...
```{r}
# Get PENIP subset
penip <- filter(counts_long_ag, !is.na(penip_site)) %>% 
