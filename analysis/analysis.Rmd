...
ggplot(fitted_taxclass, aes(x = taxon, y = fit, color = class)) +
  ...
  labs(y = "Estimated Coral Density (per m²)", x = "Taxon", color = "Size Class") +
  theme_minimal() +
  labs(title = "Bayesian Hurdle with site random")


# This output is pretty good, but some of the presumed lowest abundance taxon-size classes were unestimable,
# and the fitted values were returned as Infinity.
# I would like to find an approach that is able to produce estimates for all taxa-size classes.
```


```{r}
# Try to still use Bayesian approach for random site effects but use ZINB model instead of hurdle
library(brms)
library(future)
