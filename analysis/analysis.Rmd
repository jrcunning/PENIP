...
boot_res <- boot::boot(data = taxclass_totals, statistic = boot_pred, R = 100)
...
total_ci_zinb
# 3.22 corals per m2 (2.74-3.69 CI)
...
ggplot(taxclass_ci_zinb, aes(x = taxon, y = fit, color = class)) +
  ...
  scale_y_log10(limits = c(1e-4, 5)) +  # Set lower limit, allow auto upper limit
  coord_flip() +
...
# Compare zinb to nb
lmtest::lrtest(mod_zinb, mod_nb)  # Likelihood Ratio Test
...
# Weak priors
mod_zinb_weakpriors <- brm(
  bf(count ~ taxclass + (1 | site), zi ~ taxclass),
  family = zero_inflated_negbinomial(),
  data = penip,
  prior = c(
    prior(normal(0, 5), class = "b"),  # Weaker fixed effect priors
    prior(normal(0, 5), class = "Intercept"),  # Allow more extreme baseline density
    prior(exponential(0.5), class = "sd"),  # Allow more site variation
    prior(normal(0, 5), class = "b", dpar = "zi")  # Allow more extreme zero-inflation
  ),
  chains = 4,
  cores = 4,
  threads = threading(5),
...
# longtail taxon-abundance distribution priors
mod_zinb_longtail <- brm(
  bf(count ~ taxclass + (1 | site), zi ~ taxclass),
  family = zero_inflated_negbinomial(),
  data = penip,
  prior = c(
    prior(student_t(3, 0, 5), class = "b"),  # Heavy-tailed fixed effects
    prior(student_t(3, 0, 5), class = "Intercept"),  # Long-tailed intercept
    prior(student_t(3, 0, 3), class = "sd", lb = 0),  # Forces site variance to be positive
    prior(student_t(3, 0, 5), class = "b", dpar = "zi")  # Flexible zero-inflation
  ),
  chains = 4,
  cores = 4,
  threads = threading(5),
...
# Compare models
loo(mod_zinb_weakpriors, mod_zinb_longtail)
loo_weakpriors <- loo(mod_zinb_weakpriors)
loo_longtail <- loo(mod_zinb_longtail)
loo_strongpriors <- loo(mod_zinb_strongpriors)

bayes_factor(mod_zinb_longtail, mod_zinb_weakpriors)

pp_check(mod_zinb_weakpriors, ndraws = 100)  # Weak priors
pp_check(mod_zinb_longtail, ndraws = 100)  # Long-tailed priors
...
fitted_vals1 <- fitted(mod_zinb_longtail, newdata = newdata1, re_formula = NA) # NO SITE EFFECTS
...
fitted_vals2 <- fitted(mod_zinb_longtail, newdata = newdata1, re_formula = NULL)
...
fitted_vals3 <- fitted(mod_zinb_longtail, newdata = select(penip, taxclass, site), re_formula = NULL)
...
posterior_draws <- add_epred_draws(mod_zinb_longtail, newdata = newdata1)
