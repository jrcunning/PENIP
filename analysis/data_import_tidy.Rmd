...
newdata_dataset <- dfftaxon %>%
  distinct(dataset, taxon, class, Type, dir) %>%  # include dir
  mutate(transect_area_m2 = 1)

preds_by_dataset <- add_epred_draws(mod_nb3, newdata = newdata_dataset)

draws_dataset_avg <- preds_by_dataset %>%
...
draws_equal_weighted <- draws_dataset_avg %>%
  ...
  summarize(equal_weighted = mean(value), .groups = "drop")

summary_equal_weighted <- draws_equal_weighted %>%
  group_by(taxon, class, Type) %>%
  mean_qi(equal_weighted, .width = 0.95)
...
summary_by_class <- preds %>%
  group_by(.draw, Type, class) %>%
  summarize(total_epred = sum(.epred), .groups = "drop") %>%
  group_by(Type, class) %>%
  mean_qi(total_epred, .width = 0.95)

# Optional: Set factor levels for Type and class for better plot order
# summary_by_class$Type <- factor(summary_by_class$Type, levels = desired_order)
# summary_by_class$class <- factor(summary_by_class$class, levels = c("<4cm", "≥4cm"))

# Step 2: Plot
ggplot(summary_by_class, aes(x = Type, y = total_epred, fill = class)) +
...
tile_size <- 1000
tiles_per_row <- 50
gap_width <- 3  # space between the two columns
...
# Compute max tile column per taxon and side
col_max <- tile_expanded %>%
  group_by(taxon, side) %>%
  summarize(
    label_x = ifelse(side == "left", min(col) - 1, max(col) + 1),
    .groups = "drop"
  )
...
label_totals <- tile_data %>%
  ...
  summarize(total = sum(tot), .groups = "drop") %>%
  left_join(taxon_order, by = "taxon") %>%
  left_join(col_max, by = c("taxon", "side")) %>%
  mutate(
    label_y = taxon_y,
...
set.seed(2)
base_colors <- sample(ggsci::pal_d3("category20")(21), 21)
names(base_colors) <- unique(tot4$taxon)
...
# Plot
ggplot(summary_sus_filtered, aes(x = year, y = total_density,
                                 color = sus_group, shape = dataset, group = sus_group)) +
