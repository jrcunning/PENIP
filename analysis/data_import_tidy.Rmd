...
```{r}
# Summarize posterior draws of total coral density per habitat
...
ggplot(summary_total_density_by_type, aes(x = Type, y = mean)) +
  geom_col(fill = "gray60") +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(
    x = "Habitat Type",
    y = "Total Coral Density (per m²)",
    title = "Total Coral Density by Habitat"
  ) +
...
# ---- BY SIZE CLASS ----
summary_stacked_class <- draws_equal_weighted %>%
  group_by(.draw, Type, class) %>%
  summarize(total_epred = sum(n_m2), .groups = "drop") %>%
  group_by(Type, class) %>%
  mean_qi(total_epred, .width = 0.95) %>%
  mutate(class = factor(class, levels = c("<4cm", ">4cm"))) %>%  # new order
  arrange(Type, desc(class)) %>%
  group_by(Type) %>%
  mutate(
    stack_ymin = cumsum(lag(total_epred, default = 0)),
    stack_ymax = stack_ymin + total_epred,
    label_y = stack_ymin + total_epred / 2,
    label = sprintf("%.2f", total_epred),
    err_lower = stack_ymax - (total_epred - .lower),
    err_upper = stack_ymax + (.upper - total_epred)
  )

plot_class <- ggplot(summary_stacked_class, aes(x = Type, y = total_epred, fill = class)) +
  geom_col(position = "stack") +
  geom_errorbar(aes(ymin = err_lower, ymax = err_upper), width = 0.1, linewidth = 0.25) +
  geom_text(aes(y = label_y, label = label), size = 2.5) +
  scale_fill_manual(values = c("<4cm" = "gray75", ">4cm" = "gray60")) +
  scale_x_discrete(labels = type_labels) +
  labs(x = "Habitat Type", y = "Total Coral Density (per m²)", fill = "Size Class") +
  theme_minimal(base_size = 8) +
  theme(
...
summary_stacked_sus <- preds_grouped %>%
  group_by(.draw, Type, taxon_group) %>%
  summarize(total_epred = sum(n_m2), .groups = "drop") %>%
  group_by(Type, taxon_group) %>%
  mean_qi(total_epred, .width = 0.95) %>%
  mutate(taxon_group = factor(taxon_group, levels = c("med_sus", "low_sus", "high_sus"))) %>%
  arrange(Type, desc(taxon_group)) %>%
  group_by(Type) %>%
  mutate(
    stack_ymin = cumsum(lag(total_epred, default = 0)),
    stack_ymax = stack_ymin + total_epred,
...
plot_sus <- ggplot(summary_stacked_sus, aes(x = Type, y = total_epred, fill = taxon_group)) +
  geom_col(position = "stack") +
  geom_errorbar(aes(ymin = err_lower, ymax = err_upper), width = 0.1, linewidth = 0.25) +
  geom_text(aes(y = label_y, label = label), size = 2.5) +
  scale_fill_manual(
    #values = c("high_sus" = "#F46E6E", "med_sus" = "#FDBF6F", "low_sus" = "#A582B9"),
    values = c("high_sus" = "gray60", "med_sus" = "gray75", "low_sus" = "gray90"),
    labels = c("high_sus" = "High", "med_sus" = "Medium", "low_sus" = "Low")
  ) +
  scale_x_discrete(labels = type_labels) +
  labs(x = "Habitat Type", y = "Total Coral Density (per m²)", fill = "Disease Susceptibility") +
  theme_minimal(base_size = 8) +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 0.5),
    legend.position = "bottom",
...
ggsave(filename = "outputs/Fig3.png", width = 180, height = 90, units = "mm", dpi = 300)
...
summary_sus <- preds_grouped %>%
  ...
  mean_qi(total_epred, .width = 0.95) %>%
  mutate(
    taxon_group = factor(taxon_group, levels = c("high_sus", "med_sus", "low_sus")),
    label = sprintf("%.2f", total_epred)
  )

plot_sus <- ggplot(summary_sus, aes(x = Type, y = total_epred, fill = taxon_group)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.8) +
  geom_errorbar(aes(ymin = .lower, ymax = .upper),
                position = position_dodge(width = 0.8), width = 0.2, linewidth = 0.25) +
  #geom_text(aes(label = label), position = position_dodge(width = 0.8), vjust = -0.5, size = 2.5) +
...
combined_plot
ggsave(filename = "outputs/Fig3b.png", width = 180, height = 80, units = "mm", dpi = 300)
```
