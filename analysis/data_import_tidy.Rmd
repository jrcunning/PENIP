...
pp2 <- all_surveys_plot + facet_wrap(~ dataset, ncol = 3) + labs(title = NULL)

newfig1 <- cowplot::plot_grid(pp2, iz_plot_clean, nrow = 1, align = "h", axis = "tb", rel_widths =c(0.57, 0.43))

ggsave(filename = "outputs/Fig1.png", width = 160, height = 120, units = "mm", dpi = 300)
```
...
```{r brms}
# MODEL1 - FIT ON ALL DATA INCLUDING ALL STRUCTURAL ZEROS
# mod_nb <- brm(
#   bf(
#     n ~ dataset * taxon:class * Type + taxon:class:Type:dir +
#          (1 | dataset:site) +                       # site nested within dataset
...
#     prior(normal(-0.6, 0.75), class = "b", dpar = "shape"),
#     # Regularize random effects to prevent overfitting (important for PPP)
#     prior(exponential(2), class = "sd")     # shrinks dataset/site/dir SDs toward 0 if not supported
#   ),
...
#   bf(
#     n ~ taxon:class * Type +
#       (1 | dataset) +                            # temporal/dataset intercept
#       (1 | dataset:site) +                       # site nested within dataset
...
#   bf(
#     n ~ taxon:class * Type +
#       (1 + Type | dataset) +                     # temporal/dataset intercept
#       (1 | dataset:site) +                       # site nested within dataset
#       (1 | taxon:class:Type:dir) +               # taxon × class × Type × dir (shared geography)
#       offset(log(transect_area_m2)),
#     zi ~ taxon:class * Type,
...
#   bf(
#     n ~ taxon:class * Type +
#       (1 | dataset) +                       # dataset intercept (temporal effect)
#       (1 | dataset:site) +                  # site nested within dataset
#       (1 | taxon:class:Type:dir) +          # direction effect shared across datasets
#       offset(log(transect_area_m2)),
#     zi ~ taxon:class * Type,
#     shape ~ taxon:class
#   ),
...
#   bf(
#     n ~ taxon:class * Type +
#       (1 | dataset) +                     # temporal/dataset intercept, varies by habitat
...
#   seed = 20240717
# )
# saveRDS(mod_nb3.4, file = "data/processed/mod_nb3.4.rds")
# mod_nb3.4 <- readRDS("data/processed/mod_nb3.4.rds")
# 
# mod_nb3.5 <- brm(
#   bf(
#     n ~ taxon:class * Type +
...
#     prior(normal(-0.9, 1), class = "b", dpar = "shape"),
#     # Regularize random effects to prevent overfitting (important for PPP)
#     prior(exponential(3), class = "sd")     # shrinks dataset/site/dir SDs toward 0 if not supported
#   ),
#   chains = 4, cores = 4, threads = threading(5),
#   iter = 100, warmup = 50, thin = 1,
#   backend = "cmdstanr",
...
bayes_R2(mod_nb3.7)
#bayes_R2(mod_nb3.8)
#loo_compare(loo(mod_nb3.5), loo(mod_nb3.7))
# LOO model comparison
# loo_3.0 <- loo(mod_nb3.0)
# loo_3.1 <- loo(mod_nb3.1)
# loo_3.2 <- loo(mod_nb3.2)
# loo_3.3 <- loo(mod_nb3.3)
loo_3.4 <- loo(mod_nb3.4)
#loo_compare(loo_3.0, loo_3.1, loo_3.2, loo_3.3, loo_3.4)

loo_compare(loo_3.4, loo_2, loo_1)  # higher ELPD = better
```
...
# library(tidyr)
# 
# # 1. Posterior predictive counts (subset to 1000 draws)
# ppc_counts <- posterior_predict(
#   mod_nb3.4,
#   newdata = dfftaxon_trimmed,
#   ndraws = 200
# )
# 
# # 2. Convert to long format (fix row index)
# ppc_counts_long <- as.data.frame(ppc_counts) %>%
#   tibble::rownames_to_column("draw") %>%
#   pivot_longer(-draw, names_to = "row", values_to = "pred_count") %>%
#   mutate(
#     draw = as.integer(draw),
#     row = as.integer(gsub("[^0-9]", "", row))  # extract numeric index
#   )
# 
# # 3. Join with transect areas
# ppc_densities <- ppc_counts_long %>%
#   left_join(
#     dfftaxon_trimmed %>% mutate(row = dplyr::row_number()),
#     by = "row"
#   ) %>%
#   mutate(density = pred_count / transect_area_m2)
# 
# # 4. Summarize extreme values globally
# ppc_density_summary <- ppc_densities %>%
#   summarise(
#     q95 = quantile(density, 0.95, na.rm = TRUE),
#     q99 = quantile(density, 0.99, na.rm = TRUE),
#     max_density = max(density, na.rm = TRUE)
#   )
# 
# # 5. Summarize by taxon and class
# ppc_density_by_taxon <- ppc_densities %>%
#   group_by(taxon, class, dataset) %>%
#   summarise(
#     q95 = quantile(density, 0.95, na.rm = TRUE),
#     q99 = quantile(density, 0.99, na.rm = TRUE),
#     max_density = max(density, na.rm = TRUE),
...
# 
# ppc_density_summary
# ppc_density_by_taxon
# 

```

### Global posterior predictive checks
```{r}
library(moments)
...
set.seed(124)
yrep  <- posterior_predict(mod_nb3.7, ndraws = 200)
yrep <- sweep(yrep, 2, area, FUN = "/")
set.seed(124)
mu    <- posterior_epred(mod_nb3.7, ndraws = 200)  # matrix: draws x observations
...
ppreds <- add_predicted_draws(mod_nb3.7, newdata = dfftaxon_trimmed2, ndraws = 200) %>%
...
# Step 2: Posterior predictive draws
ppreds <- add_predicted_draws(mod_nb3.7, newdata = dfftaxon_trimmed2, ndraws = 200)

# Step 3: Simulated summaries per group and draw
pp_stats <- ppreds %>%
  group_by(taxon_class, Type, .draw) %>%
  summarize(
    mean_pred = mean(.prediction),
...
dfftaxontotals <- dfftaxon %>%
  unite("taxon_class", taxon, class) %>%
  group_by(taxon_class, Type) %>%
  summarize(ntotal = sum(n),
            nsites = sum(n > 0)) %>%
  right_join(ppp_summary_grouped)
```
...
```{r}
# Equal weighting datasets
# Step 1: Trim to taxon–class–Type combinations with at least one observation
dfftaxon_trimmed2 <- dfftaxon %>%
  semi_join(
    dfftaxon %>%
      group_by(taxon, class, Type) %>%
...
# - Dataset RE included
# - Site & dir RE marginalized automatically
...
```{r}
# Create newdata for all taxon-class-habitat combinations observed
newdata <- dfftaxon %>%
  distinct(taxon, class, Type) %>%
  mutate(transect_area_m2 = 1)

# Predict from the model - fixed effects only!
preds_model_based <- add_epred_draws(
  mod_nb3.7,
  newdata = newdata,
  re_formula = NA  # fixed effects only
)

# Summarize directly without manual weighting
summary_model_based <- preds_model_based %>%
  group_by(taxon, class, Type) %>%
...
### chatgpt's best way?
```{r}
# Step 1: Create newdata grid
# Include all taxon-class-Type combinations that were observed at least once
newdata <- dfftaxon %>%
  semi_join(
    dfftaxon %>%
      group_by(taxon, class, Type) %>%
  ...
  distinct(taxon, class, Type) %>%
  mutate(
    transect_area_m2 = 1,   # standardize to 1 m²
    dataset = NA_character_ # allows full partial pooling across datasets
  )

# Step 2: Generate posterior expected densities
preds <- add_epred_draws(
  mod_nb3.7,
  newdata = newdata,
  re_formula = NULL,             # include ALL random effects
  allow_new_levels = TRUE        # needed for new dataset levels (partial pooling)
)

# Step 3: Summarize posterior predictions
summary_densities <- preds %>%
  group_by(taxon, class, Type) %>%
  mean_qi(.epred, .width = 0.95) %>%   # posterior mean + 95% credible interval
  rename(n_m2 = .epred)

# Final table: density per taxon × class × habitat (per m²)
summary_densities
...
### Taxon densities
```{r, fig.width = 10, fig.height = 10}
densities <- summary_equal_weighted_new
# densities <- summary_model_based
# densities <- summary_densities
densities <- summary_weighted
###############
...
#### PLOT
taxdens <- ggplot(densities, aes(x = Type, y = n_m2, shape = class)) +
...
plot_total <- ggplot(summary_total_density_by_type, aes(x = Type, y = mean)) +
  ...
  scale_y_continuous(limits = c(0, 2.8), breaks = seq(0, 2.5, 0.5), minor_breaks = seq(0, 2.75, 0.25), expand = expansion(mult = c(0.02, 0))) +
  labs(
    x = "Habitat Type",
    y = "Density (per m²)",
    title = NULL
...
plot_sus <- ggplot(summary_sus, aes(x = Type, y = total_epred, fill = sus_group)) +
  ...
  #geom_text(aes(label = label), position = position_dodge(width = 0.8), vjust = -0.5, size = 2.5) +
  scale_fill_manual(
    values = c("high_sus" = "gray60", "mod_sus" = "gray75", "low_sus" = "gray90"),
    labels = c("high_sus" = "High", "mod_sus" = "Moderate", "low_sus" = "Low")
  ) +
  scale_x_discrete(labels = type_labels2) +
  scale_y_continuous(limits = c(0, 2.8), breaks = seq(0, 2.5, 0.5), minor_breaks = seq(0, 2.75, 0.25), expand = expansion(mult = c(0.02, 0))) +
  labs(
...
# diversity?
summary_equal_weighted_new %>% 
  filter(n_m2 > 0) %>% 
  group_by(Type) %>% 
  summarize(diversity = n_distinct(taxon), .groups = "drop")
```
...
tot1 <- left_join(densities, area_summary, by = "Type") %>%
...
ggsave(filename = "outputs/FigA1.png")
```
...
ggsave(filename = "outputs/FigA2.png", width = 180, height = 130, units = "mm", dpi = 600)
...
# Create newdata for prediction — use all taxa surveyed in all datasets in all habitats
newdata <- dfftaxon %>%     
  distinct(dataset, Type, taxon, class) %>%
  mutate(transect_area_m2 = 1) %>%
...
robust <- dfftaxon %>%
  group_by(dataset, taxon, class) %>%
  summarise(sites_with_obs = n_distinct(site[n > 0]), .groups = "drop") %>%
  filter(sites_with_obs > 1)

newdata2 <- newdata %>% right_join(robust)
...
# # Taxon trend tests
taxon_draw_lms <- preds_total %>%
  mutate(year = parse_number(as.character(dataset))) %>%
  filter(year >= 17) %>%      # exclude 2011 survey from trend analysis
  group_by(taxon, .draw) %>%
  summarise(
    intercept = coef(lm(total_epred ~ year))[1],
    slope = coef(lm(total_epred ~ year))[2],
    .groups = "drop"
  ) 

# 1. Generate predictions for each draw, taxon, and year (17–24)
taxon_trend_preds <- taxon_draw_lms %>%
  crossing(year = 17:24) %>%                     # add year range
  mutate(pred_density = intercept + slope * year)

# 2. Summarize across draws for ribbons (mean + 95% CI)
taxon_trend_summary <- taxon_trend_preds %>%
  group_by(taxon, year) %>%
  mean_qi(pred_density, .width = 0.95) 

taxon_trend_slopes <- taxon_draw_lms %>%
  group_by(taxon) %>%
  mean_qi(slope, .width = 0.95)


taxon_trend_slope_anno <- taxon_trend_slopes %>%
  drop_na(slope) %>%
  mutate(sign = ifelse(slope > 0, "+", "")) %>%
  mutate(label = paste0(sign, signif(slope*7, 1), " [", signif(.lower*7, 1), ",", signif(.upper*7, 1), "]"))

# taxon_percent_change_draws <- taxon_draw_lms %>%
#   mutate(
...
# 
# 
# taxon_percent_change_summary <- taxon_percent_change_draws %>%
#   group_by(taxon) %>%
#   mean_qi(percent_change, .width = 0.95) %>%
#   mutate(label = case_when(sign(.lower) != sign(.upper) ~ "No change",
#                            TRUE ~ paste0("+", round(percent_change), "% [", round(.lower), "–", round(.upper), "]")))
...
# Step 6: Plot
figA3 <- ggplot(summary_taxon_total, aes(x = year, y = total_epred)) +
  geom_line(aes(group = taxon), linewidth = 0.6, color = "gray40") +
  geom_point(aes(shape = dataset), size = 2.5, color = "black") +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0, linewidth = 0.2) +
  # Add trendlines only for years 17–24
  # geom_smooth(
  #   data = subset(summary_taxon_total, year >= 17 & year <= 24),
  #   method = "lm", se = FALSE, color = "blue", linewidth = 0.5,
  #   aes(x = year, y = total_epred), inherit.aes = FALSE
  # ) +
  ...
  theme(plot.background = element_rect(fill = "white", color = NA),
        legend.position = "bottom") +
  geom_text(data = taxon_trend_slope_anno, 
            aes(x = Inf, y = Inf, label = label), 
            inherit.aes = FALSE, color = "blue", size = 3, hjust = 1, vjust = 1) +
  scale_y_continuous(limits = c(0, NA)) +
  geom_ribbon(
    data = taxon_trend_summary,
    aes(x = year, ymin = .lower, ymax = .upper), inherit.aes = FALSE,
    alpha = 0.2
  ) +
  geom_line(
    data = taxon_trend_summary,
    aes(x = year, y = pred_density), color = "blue", inherit.aes = FALSE,
    linewidth = 1, alpha = 0.8
  ) 
figA3
ggsave(filename = "outputs/FigA3new4.png", width = 180, height = 220, units = "mm", dpi = 300)



dfftaxon %>% filter(taxon == "PORI") %>% group_by(dataset) %>% summarize(totn = sum(n), totarea = sum(transect_area_m2)) %>% mutate(dens = totn / totarea)

dfftaxon %>% filter(taxon == "ACER") %>% group_by(dataset) %>% summarize(totn = sum(n), totarea = sum(transect_area_m2)) %>% mutate(dens = totn / totarea)
...
# Prepare data: order taxa by slope
taxon_trend_slopes <- taxon_trend_slopes %>%
  drop_na(slope) %>%
  mutate(taxon = fct_reorder(taxon, slope))

# Plot slopes with 95% CI
ggplot(taxon_trend_slopes, aes(x = slope, y = taxon)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +  # reference line at 0
  geom_errorbarh(aes(xmin = .lower, xmax = .upper), height = 0.3, color = "black") +
  geom_point(aes(color = slope > 0), size = 3) +                      # color positive/negative slopes
  scale_color_manual(values = c("TRUE" = "blue", "FALSE" = "red")) +
  labs(
    x = "Slope (density change per year)",
    y = "Taxon",
    title = "Taxon-specific temporal slopes with 95% credible intervals"
  ) +
...
signed_pseudo_log_inv <- function(x, scale = 1000) {
  sign(x) * (exp(abs(x)) - 1) / scale
}
...
taxon_trend_slopes <- taxon_trend_slopes %>%
  mutate(
    slope_trans = signed_pseudo_log(slope, scale = scale_factor),
    lower_trans = signed_pseudo_log(.lower, scale = scale_factor),
    upper_trans = signed_pseudo_log(.upper, scale = scale_factor),
    taxon = fct_reorder(taxon, slope)
  )

# Tick marks (original scale)
ticks <- c(-0.5, -0.2, -0.1, -0.05, -0.02, -0.01, -0.005, -0.002, -0.001,
            -0.0001, 0, 0.0001,
            0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5)

# Plot
ggplot(taxon_trend_slopes, aes(x = slope_trans, y = taxon)) +
  geom_point() +
  geom_errorbarh(aes(xmin = lower_trans, xmax = upper_trans), height = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray40") +
  scale_x_continuous(
    breaks = signed_pseudo_log(ticks, scale = scale_factor),      # positions in transformed space
    minor_breaks = NULL,
    labels = ticks                                                # labels in original space
  ) +
  labs(
    x = "Slope (original scale, pseudo-log transformed axis)",
    y = "Taxon"
  ) +
  theme_minimal()
...
### Taxon trends with size classes
```{r, include = T, eval = T, fig.width = 10, fig.height = 6}
# Create newdata for prediction — use all taxa surveyed in all datasets in all habitats
newdata <- dfftaxon %>%     
  distinct(dataset, Type, taxon, class) %>%
...
# Step 5: Summarize posterior distribution
summary_taxon_total <- preds_avg %>%
  group_by(dataset, taxon, class) %>%
  mean_qi(mean_epred, .width = 0.95)

# # Taxon trend tests
taxon_draw_lms <- preds_avg %>%
  mutate(year = parse_number(as.character(dataset))) %>%
  filter(year >= 17) %>%      # exclude 2011 survey from trend analysis
  group_by(taxon, class, .draw) %>%
  summarise(
    intercept = coef(lm(mean_epred ~ year))[1],
    slope = coef(lm(mean_epred ~ year))[2],
    .groups = "drop"
  ) 
...
# Step 6: Plot
ggplot(summary_taxon_total, aes(x = year, y = mean_epred)) +
  geom_line(aes(group = class, linetype = class), linewidth = 0.6, color = "gray40") +
  geom_point(aes(shape = dataset, color = class), size = 2.5, color = "black") +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0, linewidth = 0.2) +
  # Add trendlines only for years 17–24
  # geom_smooth(
  #   data = subset(summary_taxon_total, year >= 17 & year <= 24),
  ...
  theme(plot.background = element_rect(fill = "white", color = NA),
        legend.position = "bottom") +
  geom_text(data = taxon_percent_change_summary, 
            aes(x = Inf, y = Inf, label = label), 
            inherit.aes = FALSE, color = "blue", size = 3, hjust = 1, vjust = 1) +
  scale_y_continuous(limits = c(0, NA)) +
  # geom_ribbon(
  ...
  geom_line(
    data = taxon_trend_summary,
    aes(x = year, y = pred_density, linetype = class, color = class), color = "blue", inherit.aes = FALSE,
    linewidth = 1, alpha = 0.8
  ) 
...
# Get total habitat-averaged densities (all taxa and size classes)
totals_filtered <- preds_avg_filtered %>%
  group_by(.draw, dataset, year) %>%
  summarize(total_density = sum(mean_epred), .groups = "drop")

# Summarize totals
summary_total <- preds_avg_filtered %>%
  group_by(.draw, dataset, year) %>%
  summarize(total_density = sum(mean_epred), .groups = "drop") %>%
...
preds_grouped <- preds_avg_filtered %>%
...
group_totals_filtered <- preds_grouped %>%
  group_by(.draw, sus_group, dataset, year) %>%
  summarize(total_density = sum(mean_epred), .groups = "drop")

summary_sus <- group_totals_filtered %>%
  group_by(sus_group, dataset, year) %>%
  mean_qi(total_density, .width = 0.95)




# Test for trends over time (within draws!)
combined_totals <- bind_rows(
  totals_filtered %>% select(.draw, dataset, year, total_density) %>% mutate(sus_group = "total"),
  group_totals_filtered %>% rename(total_density = total_density)
)

draw_lms <- combined_totals %>%
  group_by(sus_group, .draw) %>%
  summarise(
    intercept = coef(lm(total_density ~ year))[1],
    slope = coef(lm(total_density ~ year))[2],
    .groups = "drop"
  ) 

# 1. Generate predictions for each draw, sus_group, and year (17–24)
sus_trend_preds <- draw_lms %>%
  crossing(year = 17:24) %>%
  mutate(pred_density = intercept + slope * year)%>%
  mutate(sus_group = factor(sus_group, levels = c("total", "low_sus", "mod_sus", "high_sus")))

# 2. Summarize across draws for ribbons
sus_trend_summary <- sus_trend_preds %>%
  group_by(sus_group, year) %>%
  mean_qi(pred_density, .width = 0.95)%>%
  mutate(sus_group = factor(sus_group, levels = c("total", "low_sus", "mod_sus", "high_sus")))

percent_change_draws <- draw_lms %>%
  mutate(
    pred_2017 = intercept + slope * 17,
    pred_2024 = intercept + slope * 24,
    percent_change = 100 * (pred_2024 - pred_2017) / pred_2017
...
# Summarize percent change with credible intervals
percent_change_summary <- percent_change_draws %>%
  group_by(sus_group) %>%
  mean_qi(percent_change, .width = 0.95) %>%
  mutate(label = case_when(sign(.lower) != sign(.upper) ~ "No change",
                           TRUE ~ paste0("+", round(percent_change, 1), "% [", round(.lower, 1), "–", round(.upper, 1), "]")))
...
combined <- bind_rows(
  summary_total %>% select(dataset, year, total_density, .lower, .upper) %>% mutate(sus_group = "total"),
  summary_sus %>% rename(total_density = total_density)
)

combined$sus_group <- factor(combined$sus_group,
                             levels = c("total", "low_sus", "mod_sus", "high_sus"))
...
lm_labels2 <- combined %>%
  ...
  left_join(percent_change_summary %>% select(sus_group, label), by = "sus_group") %>%
  mutate(sus_group = factor(sus_group, levels = c("total", "low_sus", "mod_sus", "high_sus")))
...
# 1. Compute panel-specific y-axis limits
y_limits <- combined %>%
  group_by(sus_group) %>%
  summarize(ymin = total_density[year == 17] * 0.72) %>%
  mutate(ymax = 2.85 * ymin)

# 2. Join limits back to the plotting data
combined_with_limits <- combined %>%
  left_join(y_limits, by = "sus_group")

# 3. Plot with geom_blank to enforce limits
changeplot <- ggplot(combined_with_limits, aes(x = year, y = total_density, shape = dataset)) +
  geom_line(linewidth = 0.5, aes(group = sus_group), color = "black") +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0.1, linewidth = 0.25) +
  geom_point(size = 3, fill = "white") +
  #geom_smooth(aes(group = sus_group), method = "lm", se = FALSE, color = "blue") +
  # geom_hline(data = y_limits, aes(yintercept = ymin)) +
  # geom_hline(data = y_limits, aes(yintercept = ymax)) +
  facet_wrap(~sus_group, scales = "free", nrow = 1,
             labeller = as_labeller(c(
               "total" = "A. All corals",
               "low_sus" = "B. Low SCTLD-susc.",
  ...
  geom_blank(aes(y = ymin)) +        # enforce ymin
  geom_blank(aes(y = ymax)) +        # enforce ymax
  coord_cartesian(clip = "off")
...
ggsave(filename = "outputs/Fig5new.png", width = 180, height = 80, units = "mm")
```
...
ggplot(all, aes(x = Type)) +
  geom_point(aes(y = raw_density_mean, shape = dataset, group = dataset, color = dataset), size = 2,
             position = position_dodge(width = 0.6), alpha = 0.5) +
  geom_errorbar(aes(ymin = raw_density_lower, ymax = raw_density_upper, group = dataset, color = dataset), 
                position = position_dodge(width = 0.6), width = 0, alpha = 0.5) +
  geom_point(aes(y = n_m2), pch = 1, size = 3,
             color = "black", alpha = 0.5) +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), 
                color = "black", width = 0, alpha = 0.5) +
  facet_wrap(taxon ~ class, scales = "free", nrow = 4) +
  scale_y_continuous(limits = c(0, NA)) +
  coord_cartesian(clip = "off")
```
