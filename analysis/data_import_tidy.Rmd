...
#         plot.background = element_rect(fill = "white", color = NA))
# 
# ggsave(filename = "outputs/FigA5.png", width = 140, height = 100, units = "mm", dpi = 300)
```
...
#                      labels = c(
#     str_pad("'11", width = 3, side = "right"),
#     str_pad("'17", width = 3, side = "right"),
#     str_pad("'21", width = 6, side = "right"),
#     str_pad("'23", width = 5, side = "right"),
#     str_pad("'24", width = 5, side = "left")))+
#   theme_minimal(base_size = 11) +
#   theme(plot.background = element_rect(fill = "white", color = NA),
#         #axis.text.x = element_text(hjust = c(0.5, 0.8, 0.7, 0.2)),
#         legend.position = "bottom",
...
# 
# # Plot just a subset
# summary_datasets_subset %>%
#   ggplot(aes(x = year, y = mean_epred)) +
#   #geom_line(aes(group = taxon), linewidth = 0.6, color = "gray40") +
#   geom_point(aes(shape = dataset), size = 2.5, color = "black") +
#   geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0, linewidth = 0.2) +
#   facet_wrap(~taxon, scales = "free", ncol = 5,
#              labeller = labeller(taxon = as_labeller(taxon_labels, label_parsed))) +
#   labs(
#     x = "Year",
#     y = "Total Density (per m²)"
...
#   theme(plot.background = element_rect(fill = "white", color = NA),
#         legend.position = "bottom") +
#   geom_text(data = trends_summary_subset,
#             aes(x = Inf, y = Inf, label = str_pad(
#   ifelse(abs_change > 0,
#          paste0("+", signif(abs_change, 2)),
#          as.character(signif(abs_change, 2))),
#   width = 8,
#   side = "right"
# ),
#                 color = sign(slope.lower) == sign(slope.upper)), 
#             inherit.aes = FALSE, size = 3, hjust = 1, vjust = 1) +
#   scale_y_continuous(limits = c(0, NA)) +
#   geom_ribbon(
#     data = preds_combined_subset,
#     aes(x = year, ymin = .lower, ymax = .upper, fill = sig_trend), inherit.aes = FALSE,
#     alpha = 0.2
#   ) +
...
#   geom_line(
#     data = preds_combined_subset,
#     aes(x = year, y = pred_density, linetype = sig_trend, color = sig_trend),
#     inherit.aes = FALSE,
#     linewidth = 1, alpha = 0.8
#   ) +
