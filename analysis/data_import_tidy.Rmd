...
mod_nb3_comp <- brm(
  ...
  prior = c(
    ...
    prior(normal(1.5, 2.5), class = "b", dpar = "zi"),
    # Matches empirical shape estimates
    prior(normal(-0.6, 0.5), class = "b", dpar = "shape")
  ),
  chains = 4, cores = 4, threads = threading(5),
  iter = 200, warmup = 50, thin = 1,
  backend = "cmdstanr",
...
```{r}
# Convergence check
summary(mod_nb3)  # confirm Rhat ≈ 1.00, no divergences

# R² comparison
bayes_R2(mod_nb)
bayes_R2(mod_nb2)
bayes_R2(mod_nb3)
bayes_R2(mod_nb3_comp)

# LOO model comparison
loo_3 <- loo(mod_nb3)
loo_3_comp <- loo(mod_nb3_comp)
loo_compare(loo_3, loo_3_comp)

loo_compare(loo_3, loo_2, loo_1)  # higher ELPD = better
```
...
area <- mod_nb3_comp$data$transect_area_m2

y <- mod_nb3_comp$data$n / area
set.seed(123)
yrep  <- posterior_predict(mod_nb3_comp, ndraws = 600)
yrep <- sweep(yrep, 2, area, FUN = "/")
set.seed(123)
mu    <- posterior_epred(mod_nb3_comp, ndraws = 600)  # matrix: draws x observations
...
ppreds <- add_predicted_draws(mod_nb3_comp, newdata = dfftaxon_trimmed2, ndraws = 600) %>%
...
pp_stats %>% filter(taxon_class == "MUSS_<4cm") %>% pull(mean_pred) %>% summary(.)
...
ppreds <- add_predicted_draws(mod_nb3, newdata = dfftaxon_trimmed2, ndraws = 600)
...
preds <- add_epred_draws(mod_nb3_comp, newdata = newdata, re_formula = NULL)
...
fig2
ggsave(filename = "outputs/Fig2new6.png", width = 170, height = 170, units = "mm", dpi = 300)
...
combined_plot
ggsave(filename = "outputs/Fig3new2.png", width = 180, height = 80, units = "mm", dpi = 300)
```
...
ggsave(filename = "outputs/Fig4new3.png", width = 180, height = 130, units = "mm", dpi = 600)
...
all <- raw_density_summary %>% left_join(summary_equal_weighted)
