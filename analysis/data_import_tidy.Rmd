...
# mod_nb <- brm(
#   bf(n ~ dataset * Type * dir * taxon *class + offset(log(transect_area_m2))),
#   family = negbinomial(),
...
```{r}
# Impact zone Scenario 2
# Get all layer names
layers <- st_layers("data/Impact_zones_Scenario 2.kml")$name

# Initialize a list to store polygon names and geometries
impact_zones_data <- list()
...
for (layer_name in layers) {
  ...
  # Read the layer
  layer_data <- st_read("data/Impact_zones_Scenario 2.kml", layer = layer_name, quiet = TRUE)
  
  # Extract names and geometries
  impact_zones_data[[layer_name]] <- data.frame(
    layer = layer_name,
    name = layer_data$Name,
    geometry = st_geometry(layer_data)
  )
}

# Combine all data into a single dataframe
impact_zones_df <- bind_rows(impact_zones_data)

# View the result
print(impact_zones_df)

# Recreate sf object from dataframe
impact_zones_sf <- st_sf(impact_zones_df, crs = st_crs(polygons_clean))

# Remove Z dimension
impact_zones_2d <- st_zm(impact_zones_sf, drop = TRUE, what = "ZM")

# Check validity and fix if needed
impact_zones_valid <- st_make_valid(impact_zones_2d)

# NMFS Consideration areas
nmfs_areas <- st_read("data/Scenario2_NMFSConsiderationAreas.kml", quiet = TRUE)
nmfs_areas_2d <- st_zm(nmfs_areas, drop = TRUE, what = "ZM")
nmfs_areas_valid <- st_make_valid(nmfs_areas_2d)
...
ggplot() +
  ...
  # Impact zones (dashed lines)
  geom_sf(
    data = impact_zones_valid,
    fill = NA,
    color = "black",
    linewidth = 0.6,
    linetype = "dashed"
  ...
  # NMFS consideration areas (solid lines, different color)
  geom_sf(
    data = nmfs_areas_valid,
    fill = NA,
    color = "blue",
    linewidth = 0.8,
    linetype = "solid"
  ...
  scale_fill_brewer(palette = "Set3", na.value = "gray80") +
  theme_minimal() +
  labs(title = "Habitat, Impact Zones, and NMFS Consideration Areas", fill = "Type") +
  theme(legend.position = "right") +
...
# Combine scenario 2 impact zones with nmfs consideration areas
# Instead of selecting name (which is blank), keep the 'layer' column as the zone name
impact_zones_valid <- impact_zones_valid %>%
  mutate(name = layer) %>%   # set 'name' to the layer name
  select(name, geometry)
nmfs_areas_valid <- nmfs_areas_valid %>%
  select(name = Name, geometry)



# Combine all zone polygons into a single object
all_zones <- rbind(impact_zones_valid, nmfs_areas_valid)

unique(all_zones$name)
...
habitat_in_zones <- st_intersection(polygons_clean, all_zones)
...
# Drop habitat areas/polygons that did not intersect with any of the impact zones/consideration areas
...
# Total habitat Type area in ALL impact zones
area_summary_allzones <- area_summary %>%
  group_by(Type) %>%
...
totals <- left_join(total_perType, area_summary_allzones, by = "Type") %>%
...
# Step 3: Plot
p1 <- ggplot(totals, aes(x = Type, y = tot_estimate, fill = dataset)) +
...
```{r}
# Compute total predicted abundance by dataset/habitat/taxon/size
total_dens_taxa <- posterior_draws %>%
  # Average across direction (pool N/S) within dataset/habitat Type/taxon/class
  group_by(dataset, Type, taxon, class, .draw) %>%
  summarize(epred_dir = mean(.epred)) %>%
  # Summarize posterior draws
  group_by(dataset, Type,  taxon, class) %>%
  summarize(
    fit_mean = mean(epred_dir),
    fit_sd = sd(epred_dir),
    fit_lower = quantile(epred_dir, 0.025),
    fit_upper = quantile(epred_dir, 0.975),
    .groups = "drop"
  )

# Compute total abundances in all impact zones
totals_taxa <- left_join(total_dens_taxa, area_summary_allzones, by = "Type") %>%
  mutate(
  ...
  mutate(Type = factor(Type, levels = type_levels)) %>%
  select(dataset, Type, taxon, class, tot_estimate, tot_lower, tot_upper)


# Collapse to size class only
size_totals <- totals_taxa %>%
  group_by(dataset, class) %>%
  summarize(
    tot = sum(tot_estimate, na.rm = TRUE),
    lower = sum(tot_lower, na.rm = TRUE),
    upper = sum(tot_upper, na.rm = TRUE),
    .groups = "drop"
  )

# Plot
ggplot(size_totals, aes(x = class, y = tot, fill = dataset)) +
  geom_col(position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = lower, ymax = upper),
                position = position_dodge(width = 0.9), width = 0.2) +
  labs(x = "Size class", y = "Total corals",
       title = "Total corals per size class across datasets") +
  scale_fill_discrete(labels = dataset_labels) +
  theme_minimal()
```


# size class represented by dots
```{r}
library(tidyverse)
library(RColorBrewer)
library(colorspace)

# STEP 1: Summarize posterior predictions by dataset, Type, dir, taxon, class
total_abund_taxa <- posterior_draws %>%
  group_by(.draw, dataset, Type, dir, taxon, class) %>%
  summarize(total_epred = sum(.epred), .groups = "drop") %>%
  group_by(dataset, Type, dir, taxon, class) %>%
  summarize(
    fit_mean = mean(total_epred),
...
# STEP 2: Multiply by habitat area to get total abundance
totals_taxa <- total_abund_taxa %>%
  left_join(area_summary, by = "Type") %>%
  mutate(
    tot_estimate = fit_mean * total_area_m2,
    tot_lower = fit_lower * total_area_m2,
    tot_upper = fit_upper * total_area_m2
  ) %>%
  select(dataset, Type, ImpactZone, taxon, class, tot_estimate, tot_lower, tot_upper)

# STEP 3: Sum across cumulative impact zones
cumulative_taxa <- seq_along(impact_levels) %>%
  map_dfr(function(i) {
    totals_taxa %>%
      filter(ImpactZone %in% impact_levels[1:i]) %>%
      group_by(dataset, Type, taxon, class) %>%
      summarize(
...
# STEP 4: Keep only final cumulative total (z7)
cumulative_taxa_complete <- cumulative_taxa %>%
  filter(zone_group == paste0("z", length(impact_levels))) %>%
  complete(dataset, Type, taxon, class, fill = list(tot = 0, lower = NA, upper = NA))

# STEP 5: Create tiles
tile_size <- 5000

tile_data <- cumulative_taxa_complete %>%
  ...
  mutate(
    ...
    taxon = factor(taxon),
    taxon_class = interaction(taxon, class, sep = "_")
  ) %>%
  arrange(dataset, taxon, class) %>%
  group_by(dataset) %>%
  mutate(
    tile_id = row_number(),
    row = (tile_id - 1) %/% 40,
...
# STEP 6: Assign base taxon colors
taxa <- sort(unique(tile_data$taxon))
n_taxa <- length(taxa)
base_colors <- colorRampPalette(brewer.pal(12, "Set3"))(n_taxa)
names(base_colors) <- taxa

# STEP 7: Plot tiles with small white dots for <4cm
ggplot(tile_data, aes(x = col, y = -row, fill = taxon)) +
  geom_tile(aes(width = 0.95, height = 0.95), color = "white", linewidth = 0.1) +
  geom_point(
    data = tile_data %>% filter(class == "<4cm"),
    aes(x = col, y = -row),
    color = "white", shape = 16, size = 0.2, inherit.aes = FALSE
  ) +
  ...
  labs(
    title = "Absolute coral abundance by taxon and size class (1 tile = 5,000 corals)",
    fill = "Taxon",
...
```{r}
# Get posterior draw averages for each taxon/class/habitatType (across all datasets and N/S dir)
draw_tax_class_Type <- posterior_draws %>%
  ungroup() %>%
  select(dataset, Type, dir, taxon, class, .draw, .epred) %>%
  # Average across directions (pool N/S) within each dataset (and Type, taxon, class, .draw)
  group_by(dataset, Type, taxon, class, .draw) %>%
  summarize(dir_avg = mean(.epred), .groups = "drop") %>%
  # Average across datasets within each Type (and taxon, class,)
  group_by(.draw, Type, taxon, class) %>%
  summarize(dataset_avg = mean(dir_avg), .groups = "drop")

# Summarize posterior draws
total_taxa_class <- draw_tax_class_Type %>%
  group_by(Type, taxon, class) %>%
  summarize(
    fit_mean = mean(dataset_avg),
    fit_lower = quantile(dataset_avg, 0.025),
    fit_upper = quantile(dataset_avg, 0.975),
    .groups = "drop")

# Join area per Type and compute total abundance
total_abundance_taxa_class <- total_taxa_class %>%
  left_join(area_summary_allzones, by = "Type") %>%
  mutate(
    tot_estimate = fit_mean * total_area_m2,
    tot_lower    = fit_lower * total_area_m2,
    tot_upper    = fit_upper * total_area_m2
  ) %>%
  select(Type, taxon, class, tot_estimate, tot_lower, tot_upper)


library(tidyverse)
library(RColorBrewer)

# STEP 1: Set tile resolution
tile_size <- 5000

# STEP 2: Expand cumulative abundance into tiles
tile_data <- total_abundance_taxa_class %>%
  mutate(n_tiles = round(tot_estimate / tile_size)) %>%
  filter(n_tiles > 0) %>%
  uncount(n_tiles) %>%
  mutate(
    class = factor(class, levels = c(">4cm", "<4cm")),
...
# STEP 3: Assign distinct colors per taxon
taxa <- sort(unique(tile_data$taxon))
n_taxa <- length(taxa)
base_colors <- colorRampPalette(brewer.pal(12, "Set3"))(n_taxa)
names(base_colors) <- taxa

# STEP 4: Plot the waffle
ggplot(tile_data, aes(x = col, y = -row, fill = taxon)) +
  # Main tiles
  geom_tile(aes(width = 0.95, height = 0.95), color = "white", linewidth = 0.1) +
  
  # Subtle white dot for small size class
  geom_point(
    data = tile_data %>% filter(class == "<4cm"),
    aes(x = col, y = -row),
    color = "white", shape = 16, size = 0.2, inherit.aes = FALSE
  ...
  labs(
    title = "Average coral abundance by taxon and size class across all datasets\n(1 tile = 5,000 corals)",
    fill = "Taxon",
    caption = "White dot = smaller corals (<4cm); solid tiles = >4cm"
  )
...
# new model
```{r, include = F, eval = F}
# With this formula: ❌ You are not allowing the influence of Type on taxon × class to differ among datasets. In other words: You're assuming that the way taxon/class are distributed across habitat Types is the same in all datasets # i think this is OK
mod_nb_taxonhabitat <- brm(
  bf(n ~ dataset * taxon * class + taxon * class * Type +
         (1 | Type:dir) + (1 | dataset/site) +
...
# 2. Get fitted values manually by computing summary statistics across all draws
posterior_draws <- add_epred_draws(mod_nb_taxonhabitat, newdata = newdata, re_formula = NA) %>%
  mutate(Type = factor(Type, levels = type_levels))
...
# pairwise taxon diffs between datasets / habitats

pairwise_taxon_diffs <- posterior_draws %>%
  rename(dataset1 = dataset, epred1 = .epred) %>%
  inner_join(
    posterior_draws %>% rename(dataset2 = dataset, epred2 = .epred),
    by = c(".draw", "Type", "taxon", "class")
  ) %>%
  filter(dataset1 < dataset2) %>%
  mutate(diff = epred1 - epred2)
...
# test sctld susceptibility groups
...
# overal taxon class diffs across surveys weighted by habitat Type
# Proportion of transect area per dataset × Type × dir
survey_weights <- dfftaxon %>%
  group_by(dataset, Type) %>%
  summarize(area = sum(transect_area_m2), .groups = "drop") %>%
  group_by(dataset) %>%
  mutate(weight = area / sum(area)) %>%
  select(dataset, Type, weight)


posterior_weighted <- posterior_draws %>%
  mutate(dataset = as.character(dataset)) %>%
  left_join(survey_weights, by = c("dataset", "Type")) %>%
  mutate(weighted_epred = .epred * weight)
...
# Step 1: Sum epred across taxon and class within each dataset and habitat Type
posterior_totals <- posterior_draws %>%
  group_by(.draw, dataset, Type) %>%
  summarize(dir_total = sum(.epred), .groups = "drop")


# Step 3: Summarize across posterior draws
total_perType <- posterior_totals %>%
  group_by(dataset, Type) %>%
  summarize(
    fit_mean = mean(.epred),
    fit_sd = sd(.epred),
...
# Step 3: Plot
p1 <- ggplot(totals, aes(x = Type, y = tot, fill = dataset)) +
  geom_col(position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = lower, ymax = upper),
                position = position_dodge(width = 0.9), width = 0.2) +
  scale_fill_discrete(labels = dataset_labels) +
