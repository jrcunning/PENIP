...
# Filter to just significant trends
taxon_trend_slopes_sig <- taxon_trend_slopes %>%
  filter(sign(.lower) == sign(.upper))

# Filter to just significant trends
taxon_trend_summary_sig <- taxon_trend_summary %>% 
  filter(taxon %in% taxon_trend_slopes_sig$taxon) %>%
  mutate(taxon = factor(taxon, levels = levels(taxorder$taxon)))
...
# Step 6: Plot
figA4 <- ggplot(summary_taxon_total, aes(x = year, y = total_epred)) +
...
figA4
ggsave(filename = "outputs/FigA4.png", width = 170, height = 190, units = "mm", dpi = 300)
...
ticks <- c(-0.5, -0.2, -0.1, -0.05, -0.02, -0.01, -0.005, -0.002, -0.001,
            -0.0005, -0.0002, -0.0001, 0.0, 0.0001, 0.0002, 0.0005,
...
ggplot(taxon_trend_slopes, aes(x = slope_trans, y = taxon)) +
  ...
  labs(
    x = "Change in density (colonies m-2) 2017-2024",
    y = ""
  ) +
  theme_minimal() +
...
ggsave(filename = "outputs/FigA5.png", width = 100, height = 100, units = "mm", dpi = 300)
```
...
ggsave(filename = "outputs/FigA4.png", width = 170, height = 170, units = "mm", dpi = 300)
