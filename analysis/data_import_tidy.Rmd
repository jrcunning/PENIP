...
ggsave(filename = "outputs/FigA2new.png")
```
...
ggsave(filename = "outputs/FigA3new.png", width = 180, height = 130, units = "mm", dpi = 600)
...
ggsave(filename = "outputs/FigA4new.png", width = 180, height = 150, units = "mm", dpi = 300)
...
figA4 %+% filter(summary_taxon_total, taxon %in% c("ACER", "ORBI")) +labs(title = "")
```
...
#
```{r}
# Summarize raw densities per row
dfftaxon_with_density <- dfftaxon %>%
  mutate(density = n / transect_area_m2)

# Average density per taxon-class-Type group
raw_density_summary <- dfftaxon %>%
  ...
  mutate(
    ...
    raw_density_lower = raw_density_mean - t_crit * raw_density_se,
...
ggplot(all, aes(x = Type)) +
  geom_point(aes(y = raw_density_mean, shape = class, size = class), 
             position = position_nudge(x = -0.1), color = "blue") +
  geom_errorbar(aes(ymin = raw_density_lower, ymax = raw_density_upper), 
                position = position_nudge(x = -0.1), color = "blue", width = 0) +
  geom_point(aes(y = n_m2, shape = class, size = class), 
             position = position_nudge(x = 0.1), color = "black") +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), 
                position = position_nudge(x = 0.1), color = "black", width = 0) +
  facet_wrap(~taxon, scales = "free") +
...
# new analysis over time?
```{r}
# 1. Filter datasets
preds_filtered <- preds %>%
  filter(!dataset %in% c("nsu11_esa", "dca17_esa"))

# 2. Map dataset to year
dataset_year <- tibble(
  dataset = c("dca17", "tt21", "tt23", "drm24"),
  year    = c(17, 21, 23, 24)
)

preds_filtered <- preds_filtered %>%
  left_join(dataset_year, by = "dataset")

# 3. Average across habitat types first
preds_avg <- preds_filtered %>%
  group_by(.draw, dataset, year, taxon, class) %>%
  summarise(mean_epred = mean(.epred), .groups = "drop")

# 4. Add susceptibility groups
preds_grouped <- preds_avg %>%
  left_join(group_definitions, by = "taxon") %>%
  filter(!is.na(sus_group))

# 5. Sum densities by susceptibility group
preds_sus <- preds_grouped %>%
  group_by(.draw, dataset, year, sus_group) %>%
  summarise(density = sum(mean_epred), .groups = "drop")

# 6. Summarize posterior distributions
preds_summary <- preds_sus %>%
  group_by(dataset, year, sus_group) %>%
  mean_qi(density, .width = 0.95)


# 6. Optionally test for temporal trends using linear regression on posterior draws
trend_tests <- preds_sus %>%
  group_by(sus_group, .draw) %>%
  summarise(
    slope = coef(lm(density ~ year))[2],
    .groups = "drop"
  ) %>%
  group_by(sus_group) %>%
  mean_qi(slope, .width = 0.95)  # posterior slope estimate
