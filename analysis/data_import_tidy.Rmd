...
```{r}
# equally weighted estimates across datasets???
### dfftaxon or dfftaxon_trimmed???
...
draws_dataset_avg <- preds %>%
  group_by(.draw, taxon, class, Type, dataset, dir) %>%
  summarize(value = mean(.epred), .groups = "drop")

draws_equal_weighted <- draws_dataset_avg %>%
  group_by(.draw, taxon, class, Type) %>%
  summarize(n_m2 = mean(value), .groups = "drop")

summary_equal_weighted <- draws_equal_weighted %>%
  group_by(taxon, class, Type) %>%
  mean_qi(n_m2, .width = 0.95)

# sum(summary_equal_weighted$equal_weighted)
# sum(summary_preds$.epred)
#   
# summary_preds <- summary_equal_weighted %>% rename(.epred = equal_weighted)
```
...
# Plot of all
```{r}
posterior_type_totals <- draws_dataset_avg %>%
