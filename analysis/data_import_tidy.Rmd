...
ggplot() +
  ...
  geom_sf(data = zones_merged, aes(fill = name),
...
ggplot() +
  ...
  # Zones (with 12 manual colors and legend)
  geom_sf(data = zones_merged, aes(fill = scenario),
...
zones_split <- all_zones_sf %>%
  st_cast("POLYGON") %>%
  mutate(id = row_number())  # give each piece a unique ID
centroids <- zones_split %>%
  st_centroid(of_largest_polygon = FALSE) %>%
  mutate(X = st_coordinates(.)[,1],
         Y = st_coordinates(.)[,2])
zone_labels <- centroids %>%
  group_by(name) %>%
  slice_min(order_by = Y, n = 1, with_ties = FALSE) %>%
  ungroup()

iz_plot + 
   geom_text_repel(data = zone_labels,
                  aes(x = X, y = Y, label = name),
                  size = 3, segment.color = "gray40",
                  xlim = c(-80.08, -79),
                  max.overlaps = Inf,
                  force = 10) +
  coord_sf(
