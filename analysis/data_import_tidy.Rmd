...
set.seed(124)
yrep  <- posterior_predict(mod_nb3.7, ndraws = 100)
yrep <- sweep(yrep, 2, area, FUN = "/")
set.seed(124)
mu    <- posterior_epred(mod_nb3.7, ndraws = 100)  # matrix: draws x observations
...
ppreds <- add_predicted_draws(mod_nb3.7, newdata = dfftaxon, ndraws = 100) %>%
...
levels(as.factor(densities$class))
...
write_csv(tab2, file = "outputs/TableA3.csv")
```
