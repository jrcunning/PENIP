...
df_completed <- df_completed %>% 
  left_join(distinct(drm24_counts_agg, taxon, class, transect_area_m2), by = c("taxon", "class")) %>%
...
df_completed %>% filter(is.na(transect_area_m2), dataset == "drm24") %>%
  distinct(taxon, class)
df_completed <- df_completed %>%
  mutate(transect_area_m2 = if_else(
    dataset == "drm24" & is.na(transect_area_m2),
    20,
    transect_area_m2
  ))
