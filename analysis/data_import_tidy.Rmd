...
# drm24_counts_agg %>% 
#   filter(site %in% c("2112", "1127", "2115", "3096", "3079", "3094"), taxon %in% c("ACER", "ORBI"))
...
polygons <- st_read("data/spatial/Habitat classifications.kml")  # update path as needed
...
# 1. Convert X-position to UTM (using midpoint latitude just for reference)
...
# 2. Create tick positions in UTM
...
# 3. Convert tick positions to WGS84 (lat/lon)
...
# 4. Convert ymin and ymax to lat/lon for the two scale bar segments
...
# 5. Two scale bar segments: from ymin down and ymax up
...
# 6. Tick marks (short horizontal segments)
tick_length <- 0.0005
...
dff0 %>% filter(Type == "Artificial", taxon %in% c("ACER", "ORBI"), n > 0)
...
dfftaxon %>% filter(taxon == "ORBI", dataset == "drm24", n > 0)
dfftaxon %>% filter(taxon == "ORBI", dataset == "dca17", n > 0)
dfftaxon %>% filter(taxon == "ORBI", dataset == "tt21", n > 0)
dfftaxon %>% filter(taxon == "ORBI", dataset == "tt23", n > 0)
```
...
# Look at 2011 -- 2017 trends for the 3 spp
...
data("SEFCRI_2024_coral_demographics")

SEFCRI_2024_coral_demographics$MAX_DIAMETER
SEFCRI_2024_coral_demographics$JUV
SEFCRI_2024_coral_demographics$HEIGHT
SEFCRI_2024_coral_demographics$SPECIES_NAME

data("NCRMP_SEFCRI_2014_24_density_species")
View(NCRMP_SEFCRI_2014_24_density_species)


nc <- read_csv("/Users/rcunning/Downloads/0306238/1.1/data/0-data/Data_Sets/NCRMP_SEFCRI2024_Benthic_Data02_CoralDemographics.csv") %>%
  janitor::clean_names()
allsites <- nc %>% distinct(primary_sample_unit)

nca <- nc %>%
  filter(juv == 0) %>%
  right_join(allsites) %>%
  replace_na(list(n = 0))
#ncj0 <- filter(nc, juv == 1)
ncj <- nc %>%
  filter(juv == 1) %>%
  right_join(allsites) %>%
  replace_na(list(n = 0))
#n_distinct(ncj0$primary_sample_unit)
#n_distinct(ncj$primary_sample_unit)
...
ncadens <- nca %>%
  ...
  summarize(n = sum(n)) %>%
  mutate(n_m2 = n / 10) %>%
  filter(primary_sample_unit != 3100)
mean(ncadens$n_m2)
hist(ncadens$n_m2)
```


# Reanalyze Jones and Gilliam data with 4cm threshold
```{r}
jg_siteinfo <- read_csv("https://raw.githubusercontent.com/nj350/Recruitment-Size-Structure---Frontiers/refs/heads/main/Site%20Info.csv") %>%
  janitor::clean_names()

distinct(jg_siteinfo, latitude, longitude, habitat) %>%
  ggplot(aes(x = longitude, y = latitude, color = habitat)) +
  geom_point()

jg_adults <- read_csv("https://raw.githubusercontent.com/nj350/Recruitment-Size-Structure---Frontiers/refs/heads/main/All%20Colonies.csv") %>%
  janitor::clean_names()

jg_juv <- read_csv("https://raw.githubusercontent.com/nj350/Recruitment-Size-Structure---Frontiers/refs/heads/main/All%20Juveniles.csv") %>%
  janitor::clean_names()

jg_juv4 <- jg_adults %>%
  filter(pmax(colony_length, colony_width, colony_height) < 4) %>%
  group_by(survey_year, year, site, species) %>%
  summarize(abundance = n())

jg_juv_combined <- bind_rows(jg_juv, jg_juv4) %>%
  group_by(survey_year, year, site, species) %>%
  summarise(abundance = sum(abundance, na.rm = TRUE), .groups = "drop") %>%
  left_join(jg_siteinfo)

jg_juv_summ <- jg_juv_combined %>%
  group_by(survey_year, year, site, habitat) %>%
  summarize(tot_juv = sum(abundance), .groups = "drop") %>%
  mutate(tot_juv_dens = tot_juv / 30) %>%
  filter(site != "FTL6")

jg_juv_summ %>%
  ggplot(aes(x = tot_juv_dens)) +
  geom_histogram()
mean(jg_juv_summ$tot_juv_dens)
median(jg_juv_summ$tot_juv_dens)


jg_adults_summ <- jg_adults %>%
  filter(pmax(colony_length, colony_height, colony_height) >= 4) %>%
  count(survey_year, year, site) %>%
  mutate(tot_adult_dens = n / 30) %>%
  left_join(jg_siteinfo) %>%
  filter(site != "FTL6")

jg_adults_summ %>%
  ggplot(aes(x = tot_adult_dens)) +
  geom_histogram()

mean(jg_adults_summ$tot_adult_dens)
median(jg_adults_summ$tot_adult_dens)
```
