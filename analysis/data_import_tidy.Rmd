...
# PLOT SLOPES / CHANGE OVER TIME

# Prepare data: order taxa by slope
taxon_trend_slopes2 <- taxon_trend_slopes %>%
  drop_na(slope) %>%
  mutate(across(c(slope, .lower, .upper), ~ . * 7)) %>%
...
# Transform slopes
scale_factor <- 2500
taxon_trend_slopes2<- taxon_trend_slopes2 %>%
  mutate(
...
ticks <- c(-0.5, -0.2, -0.1, -0.05, -0.02, -0.01, -0.005, -0.002, -0.001,
            -0.0005, -0.0001, 0.0, 0.0001, 0.0005,
...
# Plot
ggplot(taxon_trend_slopes2, aes(x = slope_trans, y = taxon)) +
  geom_point() +
...
trend_preds <- draw_lms %>%
  crossing(year = 17:24) %>%
  mutate(pred_density = intercept + slope * year)

# 2. Summarize across draws for ribbons
trend_summary <- trend_preds %>%
  group_by(year) %>%
  mean_qi(pred_density, .width = 0.95)

pred_change_draws <- draw_lms %>%
  mutate(
    pred_2017 = intercept + slope * 17,
    pred_2024 = intercept + slope * 24,
    pred_change = pred_2024 - pred_2017
  )

# Summarize percent change with credible intervals
pred_change_summary <- pred_change_draws %>%
  mean_qi(pred_change, .width = 0.95) %>%
  mutate(label = case_when(sign(.lower) != sign(.upper) ~ "No change",
                           TRUE ~ paste0("+", signif(pred_change, 2), " [", signif(.lower, 2), "–", signif(.upper, 2), "]")))
...
lm_labels2 <- summary_total %>%
  summarize(
    x = min(year),
    y = max(total_density),
    .groups = "drop"
  ) %>%
  bind_cols(pred_change_summary)
