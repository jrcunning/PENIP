...
p2
### NOT ALL HABITATS OR TAXA WERE SURVEYED IN ALL DATASETS
...
# Coral densities - ACROSS ALL datasets, with zero-inflated negbin model
```{r}
# 🎯 Goal
# 
...
summary_preds_fmt <- summary_preds %>%
  mutate(
    ...
    mean_ci = case_when(
      .epred < 0.001 ~ sprintf("%.1e [%.1e–%.1e]", .epred, .lower, .upper),
      TRUE ~ sprintf("%.4f [%.4f–%.4f]", .epred, .lower, .upper)
    )
  )
