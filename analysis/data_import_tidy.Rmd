...
# Compute and plot total coral abundance by dataset, habitat Type, and direction from channel
# Step 1: Sum epred across taxon and class within each habitat Type and direction
...
```{r}
# Compute total predicted abundance by taxon/size
...
totals_taxa <- left_join(total_abund_taxa, area_summary, by = "Type") %>%
  mutate(
...
size_totals <- cumulative_taxa_complete %>%
...
# Collapse to total per taxon per dataset
taxon_totals <- cumulative_taxa_complete %>%
  group_by(dataset, taxon) %>%
  summarize(tot = sum(tot, na.rm = TRUE), .groups = "drop")

# Normalize within each dataset for relative abundance
taxon_props <- taxon_totals %>%
  group_by(dataset) %>%
  mutate(prop = tot / sum(tot)) %>%
  ungroup()

tile_resolution <- 1000  # Each tile = 0.1%

waffle_data_1000 <- taxon_props %>%
  group_by(dataset, taxon) %>%
  mutate(n_tiles = round(prop * tile_resolution)) %>%
  ungroup() %>%
  uncount(n_tiles) %>%
  group_by(dataset) %>%
  mutate(tile_id = row_number()) %>%
  filter(tile_id <= tile_resolution) %>%  # enforce exactly 1000 tiles
  mutate(
    row = (tile_id - 1) %/% 40,  # 40 columns (25 rows)
    col = (tile_id - 1) %% 40
  )

ggplot(waffle_data_1000, aes(x = col, y = -row, fill = taxon)) +
  geom_tile(color = "white", size = 0.1) +
  facet_wrap(~ dataset) +
  scale_fill_manual(values = colorRampPalette(RColorBrewer::brewer.pal(8, "Set3"))(length(unique(waffle_data_1000$taxon)))) +
  coord_equal() +
  theme_void() +
  labs(
    title = "Taxon composition per dataset (each tile = 0.1%)",
    fill = "Taxon"
  )

# WAFFLES WITH TOTAL COUNTS RATHER THAN PROPORTIONS
tile_size <- 5000

tile_data <- taxon_totals %>%
  ...
  mutate(tile_id = row_number()) %>%
  mutate(
    row = (tile_id - 1) %/% 40,  # 40 columns wide; ~square layout
    col = (tile_id - 1) %% 40
  )
...
ggplot(tile_data, aes(x = col, y = -row, fill = taxon)) +
  ...
  scale_fill_manual(
    values = colorRampPalette(RColorBrewer::brewer.pal(8, "Set3"))(length(unique(tile_data$taxon)))
  ) +
  coord_equal() +
  theme_void() +
  theme(
  ...
  guides(
    fill = guide_legend(nrow = 2, byrow = TRUE)  # adjust nrow if needed
  ) +
  labs(
    title = "Absolute coral abundance per dataset (1 tile = 5,000 corals)",
    fill = "Taxon"
  )
...
waffle plot by taxon and size class
```{r}
library(tidyverse)
library(RColorBrewer)
library(colorspace)

# STEP 1: Aggregate total coral abundance by dataset, taxon, and class
tile_base <- cumulative_taxa_complete %>%
  group_by(dataset, taxon, class) %>%
  summarize(tot = sum(tot, na.rm = TRUE), .groups = "drop") %>%
  filter(tot > 0)

# STEP 2: Choose tile size and create tile rows
tile_size <- 5000  # 1 tile = 5,000 corals
...
tile_data <- tile_base %>%
  ...
  mutate(
    row = (tile_id - 1) %/% 40,
    col = (tile_id - 1) %% 40,
    taxon_class = interaction(taxon, class, sep = "_")  # unique ID for color mapping
  )

# Step 1: Get taxon list
taxa <- sort(unique(tile_data$taxon))
n_taxa <- length(taxa)  # should be 16

# Step 2: Use a palette that supports 16+ visually distinct colors
base_colors <- colorRampPalette(brewer.pal(12, "Set3"))(n_taxa)
names(base_colors) <- taxa

# Step 3: Define mild light/dark variation per class
get_shades <- function(color) {
  unname(c(lighten(color, 0.03), darken(color, 0.03)))  # now guaranteed clean
}

class_levels <- sort(unique(tile_data$class))  # e.g., "small", "large"

shade_colors <- map_dfr(seq_along(taxa), function(i) {
  this_taxon <- taxa[[i]]
  this_color <- base_colors[[this_taxon]]
  
  tibble(
    taxon = rep(this_taxon, 2),
    class = class_levels,
    color = get_shades(this_color)
  )
}) %>%
...
# Step 5: Create named vector
color_vec <- setNames(shade_colors$color, shade_colors$taxon_class)




waffle_plot <- ggplot(tile_data, aes(x = col, y = -row, fill = taxon_class)) +
  geom_tile(color = "white", size = 0.1) +
  facet_grid(~ dataset, scales = "fixed") +
...
# Create legend dataframe: one row per taxon × size class
legend_df <- shade_colors %>%
  mutate(
    size_label = ifelse(class == class_levels[1], "Small", "Large"),
    row = match(taxon, taxa),
    col = ifelse(size_label == "Large", 1, 2)  # col 1 = large, col 2 = small
  )
# Plot the legend using geom_tile
legend_plot <- ggplot(legend_df, aes(x = col, y = -row, fill = color)) +
  geom_tile(color = "white", width = 0.9, height = 0.9) +
  scale_fill_identity() +
  # Taxon names next to the row
  geom_text(aes(x = 2.6, label = taxon), hjust = 0, size = 3.5) +
  # Column labels at the top
  annotate("text", x = 1, y = 1.5, label = "Large", size = 4.5, fontface = "bold") +
  annotate("text", x = 2, y = 1.5, label = "Small", size = 4.5, fontface = "bold") +
  coord_fixed() +
  theme_void() +
  theme(
    plot.margin = margin(10, 10, 10, 10)
  ) +
  labs(title = "Taxon and Size Class Legend")


waffle_plot + legend_plot + plot_layout(widths = c(4, 1))
```

# size class represented by different tile size
```{r}
library(tidyverse)
library(RColorBrewer)
library(colorspace)
...
# STEP 3: Set taxon and class orders
taxa <- sort(unique(tile_base$taxon))
class_levels <- c(">4cm", "<4cm")  # large first

# STEP 4: Expand tiles and define order
tile_data <- tile_base %>%
  mutate(n_tiles = round(tot / tile_size)) %>%
  filter(n_tiles > 0) %>%
  uncount(n_tiles) %>%
  mutate(
    ...
    taxon_class = factor(paste(taxon, class, sep = "_"),
                         levels = as.vector(outer(taxa, class_levels, paste, sep = "_")))
  ) %>%
  arrange(dataset, taxon, class) %>%  # <== THIS LINE FIXES TILE ORDER
  group_by(dataset) %>%
  mutate(
    tile_id = row_number(),
    row = (tile_id - 1) %/% 40,
...
# STEP 5: Assign base colors to taxa
n_taxa <- length(taxa)
base_colors <- colorRampPalette(brewer.pal(12, "Set3"))(n_taxa)
names(base_colors) <- taxa

# STEP 6: Assign tile size by class
tile_data <- tile_data %>%
  mutate(
    tile_width = ifelse(class == "<4cm", 0.85, 0.95),
    tile_height = ifelse(class == "<4cm", 0.85, 0.95)
  )

# STEP 7: Plot
ggplot(tile_data, aes(x = col, y = -row, fill = taxon)) +
  geom_tile(aes(width = tile_width, height = tile_height), color = "white", linewidth = 0.1) +
  facet_grid(~ dataset, scales = "fixed") +
  ...
  labs(
    title = "Absolute coral abundance by taxon and size class (1 tile = 5,000 corals)",
    fill = "Taxon",
    caption = "Smaller tiles = smaller corals (<4cm); larger tiles = >4cm"
  )
