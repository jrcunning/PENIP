...
p <- ggplot() +
  ...
  #scale_fill_brewer(palette = "Set3", na.value = "gray80") +
  scale_fill_manual(values = pal, na.value = "gray80") +
...
# KJ Coral Reef ECA (official DEP layer via ArcGIS REST → GeoJSON)
kj_url <- "https://ca.dep.state.fl.us/arcgis/rest/services/OpenData/KJ_CORAL_REEF_ECA/MapServer/0/query?where=1=1&outFields=*&outSR=4326&f=geojson"

kj_eca <- st_read(kj_url, quiet = TRUE)  # already WGS84 (outSR=4326)

# (Optional) crop to your current view bbox if you have one defined
# kj_eca <- st_crop(kj_eca, bbox_view)
