...
# Or also include Outer Reef and Aggregated Patch Reef?
...
### 3.2.2. By habitat, taxon, and survey
...
# Plot
ggplot(fitted_manual, aes(y = fit_mean, x = taxon, color = dir)) +
...
ggplot(fitted_manual, aes(y = fit_mean, x = Type, color = dir, shape = dataset,
...
posterior_NS_diff <- posterior_draws %>%
  ungroup() %>%
...
dir_comparisons <- posterior_NS_diff %>%
...
# Plot total coral abundance
fitted_total <- posterior_draws %>%
...
ggplot(fitted_total, aes(x = dataset, y = fit_mean, color = dir)) +
  facet_grid(~ Type) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = fit_lower, ymax = fit_upper), width = 0,
...
posterior_NS_total_diff <- posterior_draws %>%
  ungroup() %>%
  group_by(.draw, dataset, Type, dir) %>%
  summarize(total_epred = sum(.epred), .groups = "drop") %>%
...
total_diff_summary <- posterior_NS_total_diff %>%
...
# 2. Get posterior draws of fitted values on response scale (no random effects)
fitted_draws <- fitted(
  mod_nb,
  newdata = newdata,
  re_formula = NA,
...
# 3. Reshape into long format and join with newdata
draws_long <- as.data.frame(fitted_draws) %>%
  mutate(.draw = row_number()) %>%
  pivot_longer(cols = starts_with("V"), names_to = "col", values_to = "fit") %>%
  mutate(row_id = as.integer(gsub("V", "", col))) %>%
  left_join(newdata, by = "row_id")

# 4. Draw-wise sum across taxon to get total density per dataset × Type × draw
total_draws <- draws_long %>%
  group_by(.draw, dataset, Type) %>%
  summarize(total = sum(fit), .groups = "drop") %>%
  mutate(taxon = "Total")

# 5. Summarize across posterior draws to get mean, SE, and 95% credible intervals
total_summ <- total_draws %>%
  group_by(dataset, Type, taxon) %>%
  summarize(
    fit   = mean(total),
...
# 6. (Optional) Combine with taxon-specific summaries from brms::fitted()
fitted_taxon <- fitted(mod_nb, newdata = newdata, re_formula = NA, summary = TRUE) %>%
  as_tibble() %>%
  bind_cols(newdata) %>%
  rename(
    fit = Estimate,
...
combined_fitted <- bind_rows(fitted_taxon, total_summ)

ggplot(combined_fitted, aes(x = log10(fit), y = taxon)) +
  geom_point() +
  geom_errorbarh(aes(xmin = log10(lower), xmax = log10(upper)), height = 0.2) +
  facet_grid(Type ~ dataset) +
  labs(
    x = expression("log"[10] * " coral density (per m²)"),
    y = NULL,
    title = "Predicted coral density by taxon, dataset, and habitat type"
  ) +
  theme_minimal() +
...
# now test for diffs between surveys within Type/taxa?
```{r}
library(posterior)

# 1. Clean draws: one row per draw × dataset × taxon × Type
draws_clean <- draws_long %>%
  filter(!is.na(taxon), taxon != "Total") %>%
  select(.draw, dataset, Type, taxon, fit)

# 2. Pairwise comparisons within each .draw × Type × taxon group
pairwise_diffs <- draws_clean %>%
  group_by(.draw, Type, taxon) %>%
  inner_join(draws_clean, by = c(".draw", "Type", "taxon")) %>%
  ...
  select(.draw, Type, taxon, dataset1, dataset2, diff) %>%
  ungroup()

# 3. Summarize posterior differences
contrast_summary <- pairwise_diffs %>%
  group_by(Type, taxon, dataset1, dataset2) %>%
  summarize(
    mean_diff = mean(diff),
    lower = quantile(diff, 0.025),
    upper = quantile(diff, 0.975),
    p_nonzero = mean(diff > 0),
    .groups = "drop"
  ) %>%
  mutate(
    p_direction = ifelse(mean_diff > 0, p_nonzero, 1 - p_nonzero),
    significant = p_direction > 0.95
  )

contrast_summary %>%
  arrange(Type, taxon, dataset1, dataset2)
...
mod_nb11 <- glmmTMB(
  n ~ dataset * Type * taxon + offset(log(transect_area_m2)),
  data = dfftaxon,
  family = nbinom2()
)
summary(mod_nb11)

newdata <- mod_nb11$frame %>%
  select(dataset, Type, taxon) %>%
  distinct() %>%
  mutate(transect_area_m2 = 1)

preds <- predict(mod_nb11, newdata = newdata, type = "response", se.fit = TRUE)
...
# 1. Get model frame with predicted values
fitted_df <- as_tibble(mod_nb11$frame) %>%
  mutate(fit_transect = predict(mod_nb11, type = "response")) %>%
  # Bind in original data to restore area info
  bind_cols(transect_area_m2 = dfftaxon_trimmed$transect_area_m2) %>%
  # convert to per m²
  mutate(fit = fit_transect / transect_area_m2)

# 2. Summarize taxon-specific coral density per m²
taxon_df <- fitted_df %>%
  group_by(dataset, Type, taxon) %>%
...
# 3. Add total coral density across taxa
total_df <- taxon_df %>%
  group_by(dataset, Type) %>%
  summarize(taxon = "Total",
            fit = sum(fit),
            se = sqrt(sum(se^2)),
            lower = pmax(fit - 1.96 * se, 1e-4),
            upper = fit + 1.96 * se,
...
# 4. Combine
combined_df <- bind_rows(taxon_df, total_df)

# 5. Keep only observed taxon combinations
valid_combos <- dfftaxon %>%
  group_by(dataset, Type, taxon) %>%
  summarize(total_n = sum(n), .groups = "drop") %>%
  filter(total_n > 0)

plot_df <- bind_rows(combined_df %>% filter(taxon == "Total"),
                     combined_df %>% filter(taxon != "Total") %>%
           inner_join(valid_combos, by = c("dataset", "Type", "taxon")))

# 6. Plot log10 density
ggplot(plot_df, aes(x = log10(fit), y = taxon)) +
  geom_point() +
  geom_errorbarh(aes(xmin = log10(lower), xmax = log10(upper)), height = 0.2) +
  facet_grid(Type ~ dataset) +
  labs(
    x = expression("log"[10] * " coral density (per m²)"),
    y = NULL,
    title = "Coral density by taxon, dataset, and habitat type"
  ) +
  theme_minimal() +
  theme(strip.text = element_text(face = "bold"))
```
...
# Extract and plot taxon--specific densities
taxclass_ci_nb <- results_nb %>%
  dplyr::select(dataset, Type, taxon, fit, fit_se, fit_lower, fit_upper)

# Compute total density per taxon 
fitted_taxon_nb <- taxclass_ci_nb %>%
  group_by(dataset, Type, taxon) %>%
  summarize(
    fit = sum(fit),  # Sum densities across size classes
...
avg_density_ci <- total_ci_nb %>%
  ...
  summarize(
    ...
    avg_density = sum(total_density * w) / sum(w),
    
    # Standard error of the weighted mean
    se_density = sqrt(1 / sum(w)),
    
    # Confidence intervals
    lower_95CI = avg_density - 1.96 * se_density,
    upper_95CI = avg_density + 1.96 * se_density,
    
    .groups = "drop"
  )

tots <- avg_density_ci %>%
  distinct(Type, avg_density, lower_95CI, upper_95CI) %>%
  full_join(area_summary2) %>%
  mutate(totlow = lower_95CI * total_area_m2,
         totup = upper_95CI * total_area_m2) %>%
  group_by(ImpactZone) %>%
  summarize(totlow = sum(totlow),
            totup = sum(totup))
