...
cts <- dfftaxon %>%
  count(taxon, class)
#patchiness
vmr_by_habitat <- dfftaxon %>%
  filter(!is.na(transect_area_m2), transect_area_m2 > 0) %>%
  mutate(density = n / transect_area_m2) %>%
  group_by(taxon, class, Type) %>%
  summarize(
    mean_density = mean(density),
    var_density  = var(density),
    VMR          = var_density / mean_density,
    n_sites      = n(),
    .groups = "drop"
  )
...
dfftaxon %>%
  filter(!is.na(transect_area_m2), transect_area_m2 > 0) %>%
  mutate(density = n / transect_area_m2) %>%
  ggplot(aes(x = density, fill = class)) +
  geom_histogram() +
...
vmr_by_habitat %>% arrange(-VMR) %>% print(n = nrow(.))
summary(vmr_by_habitat)
ggplot(vmr_by_habitat, aes(x = taxon, y = VMR)) +
  geom_point(aes(shape = Type, color = Type, size = class), alpha = 0.5) +
  scale_y_log10() +
...
# Create newdata grid for taxon:class:Type
newdata_zi <- dfftaxon %>%
  distinct(taxon, class, Type) %>%
  mutate(transect_area_m2 = 1)

# Posterior draws of zero-inflation probabilities
zi_preds <- add_linpred_draws(
  mod_nb3,
  newdata = newdata_zi,
  dpar = "zi",
  transform = TRUE,  # Applies plogis link
  re_formula = NA    # Use population-level effects
)
...
zi_summary <- zi_preds %>%
  ...
  summarize(
    zi_mean = mean(zi),
    zi_lower = quantile(zi, 0.025),
    zi_upper = quantile(zi, 0.975),
    .groups = "drop"
  )


vmr_vs_zi <- vmr_by_habitat %>%
  left_join(zi_summary, by = c("taxon", "class", "Type"))

ggplot(vmr_vs_zi, aes(x = VMR, y = zi_mean)) +
  geom_point(aes(size = mean_n), alpha = 0.7) +
  geom_errorbarh(aes(xmin = VMR * 0.95, xmax = VMR * 1.05), height = 0, alpha = 0.3) +
  geom_errorbar(aes(ymin = zi_lower, ymax = zi_upper), width = 0, alpha = 0.3) +
  scale_x_log10() +
  labs(
    x = "Variance-to-Mean Ratio (VMR, log scale)",
    y = "Posterior Mean Zero-Inflation Probability",
    title = "Empirical Patchiness vs. Model-Inferred Zero-Inflation",
    size = "Mean Density"
  ) +
  theme_minimal() +
  geom_text(aes(label = taxon))

zi_summary %>% arrange(-zi_mean) %>% print(n = nrow(.))
```


```{r}
# fixed version of equally weighting estimates across dataset-averaging? 
...
preds <- add_epred_draws(mod_nb3, newdata = newdata, re_formula = NULL)
...
#### PLOT
(taxdens <- ggplot(summary_equal_weighted, aes(x = Type, y = n_m2, color = class)) +
  geom_point(size = 1) +
...
```{r}
summary_totals <- draws_equal_weighted %>%
  group_by(.draw, Type) %>%
...
# By SiZE CLASS by HABITAT
# Step 1: Summarize posterior predictions
summary_by_class <- draws_equal_weighted %>%
  group_by(.draw, Type, class) %>%
  summarize(total_epred = sum(n_m2), .groups = "drop") %>%
  group_by(Type, class) %>%
  mean_qi(total_epred, .width = 0.95)

# Step 1: Force stacking order: <4cm on top, >4cm on bottom
summary_stacked <- summary_by_class %>%
  mutate(class = factor(class, levels = c("<4cm", ">4cm")))  # <4cm will be stacked last = on top

# Step 2: Compute stacking positions in reverse of factor order
summary_stacked <- summary_stacked %>%
  arrange(Type, desc(class)) %>%  # arrange so >4cm is stacked first (bottom)
  group_by(Type) %>%
  mutate(
    stack_ymin = cumsum(lag(total_epred, default = 0)),
    stack_ymax = stack_ymin + total_epred,
    err_lower = stack_ymax - (total_epred - .lower),
    err_upper = stack_ymax + (.upper - total_epred)
  ) 

# Plot
ggplot(summary_stacked, aes(x = Type, y = total_epred, fill = class)) +
  geom_col(position = "stack") +
  geom_errorbar(
    aes(ymin = err_lower, ymax = err_upper), linewidth = 0.25,
    width = 0.1
  ) +
  scale_fill_manual(values = c("<4cm" = "gray70", ">4cm" = "gray40")) +
  scale_x_discrete(labels = c(
    "Nearshore Ridge Complex" = "Nearshore\nRidge Complex",
...
ggsave(filename = "outputs/Fig3.png", width = 80, height = 80, units = "mm", dpi = 300)



# By SUS GROUP OVERALL

group_definitions <- tibble(
  taxon = unique(draws_equal_weighted$taxon),
  taxon_group = case_when(
    taxon %in% c("FAVI", "MUSS", "MEAN", "DSTO", "EFAS", 
                 "MMEA", "DLAB", "CNAT", "FAVI", "PSTR", "PCLI") ~ "high_sus",
    taxon %in% c("MCAV", "ORBI", "SIDE", "SINT", "SOLE") ~ "med_sus",
    taxon %in% c("ACER", "PORI", "AGAR", "MYCE", "MADR") ~ "low_sus",
    TRUE ~ NA_character_
  )
)
# Join susceptibility group definitions
preds_grouped <- draws_equal_weighted %>%
  left_join(group_definitions, by = "taxon") %>%
  filter(!is.na(taxon_group))

# Summarize posterior predictions by draw × Type × group
summary_grouped <- preds_grouped %>%
  group_by(.draw, Type, taxon_group) %>%
  summarize(total_epred = sum(n_m2), .groups = "drop") %>%
  group_by(Type, taxon_group) %>%
  mean_qi(total_epred, .width = 0.95)

# Optional: order groups for plot aesthetics
summary_grouped$taxon_group <- factor(summary_grouped$taxon_group,
                                      levels = c("high_sus", "med_sus", "low_sus"))

# Plot
ggplot(summary_grouped, aes(x = Type, y = total_epred, fill = taxon_group)) +
  ...
  scale_fill_manual(
    values = c("high_sus" = "#E31A1C", "med_sus" = "#FDBF6F", "low_sus" = "#6A3D9A"),
...
ggsave(filename = "outputs/FigAX.png")
```
...
ggsave(filename = "outputs/Fig4b2.png", width = 180, height = 130, units = "mm", dpi = 600)
...
ggplot(summary_taxon_total, aes(x = year, y = total_epred)) +
  ...
  #scale_y_log10()+
  scale_x_continuous(breaks = seq(11,24,2)) +
...
## Sus-group densities across datasets
```{r, fig.width = 10, fig.height = 5}
# Step 1: Define susceptibility groups
group_definitions <- tibble(
...
# Step 2: Create newdata with all combos (retain Type)
newdata_ds <- dfftaxon_trimmed2 %>%
  distinct(dataset, Type, taxon, class) %>%
  mutate(transect_area_m2 = 1, dir = NA)

# Step 3: Predict densities (posterior draws)
...
# Step 4: Average across habitat Type per draw, taxon, class, dataset
preds_avg <- preds_ds %>%
  group_by(.draw, taxon, class, dataset) %>%
  summarize(mean_epred = mean(.epred), .groups = "drop")
...
# Plot
suschange <- ggplot(summary_sus_filtered, aes(x = year, y = total_density,
                                 shape = dataset, group = sus_group)) +
  ...
  facet_wrap(~sus_group, scales = "free",
               labeller = as_labeller(c(
    "high_sus" = "High susceptibility",
    "med_sus" = "Moderate susceptibility",
    "low_sus" = "Low susceptibility"
  ))) +
  guides(color = "none") +
  theme(
    legend.position = "bottom"
...
lm_stats <- summary_sus_filtered %>%
  group_by(sus_group) %>%
  do(broom::tidy(lm(total_density ~ year, data = .))) %>%
  filter(term == "year") %>%  # only keep slope (year) estimates
  mutate(
    label = paste0("Slope = ", signif(estimate, 2), 
                   ", p = ", signif(p.value, 2))
  )

lm_labels <- summary_sus_filtered %>%
  group_by(sus_group) %>%
  summarize(
    x = min(year),
    y = max(total_density),
  ...
  left_join(lm_stats, by = "sus_group")
...
suschange + 
  ...
  theme(
    ...
    strip.text = element_text(face = "bold")  # optional: makes label text bold
  ) +
  theme(
    legend.position = "bottom",
    legend.margin = margin(t = 0, r = 0, b = 0, l = 0),         # shrink padding inside legend
    legend.box.margin = margin(t = -5, r = 0, b = 0, l = 0)     # pull legend box closer to plot
  )
  
ggsave(filename = "outputs/Fig5.png", width = 180, height = 80, units = "mm", dpi = 300)
```

```{r}
# TOTAL coral density over time/datasets
# Step 1: Create newdata with all needed combinations (retain Type)
newdata_total <- dfftaxon_trimmed2 %>%
  distinct(dataset, Type, taxon, class) %>%
  mutate(transect_area_m2 = 1, dir = NA)

# Step 2: Predict densities including group-level effects
preds_total_ds <- add_epred_draws(
  mod_nb3,
...
# Step 3: Average across habitat Type per draw, taxon, class, dataset
preds_total_avg <- preds_total_ds %>%
  group_by(.draw, taxon, class, dataset) %>%
  summarize(mean_epred = mean(.epred), .groups = "drop")

# Step 4: Sum total predicted density (all taxa and size classes)
summary_total <- preds_total_avg %>%
...
# Step 6: Plot
ggplot(summary_total_filtered, aes(x = year, y = total_density, shape = dataset)) +
  geom_line(linewidth = 0.5, color = "black", group = 1) +
  ...
  labs(
    x = "Year",
    y = "Total Coral Density (per m²)",
    title = "Total Coral Density Across Datasets"
  ) +
  scale_shape_manual(values = c(21, 22, 24, 25)) +
  theme_classic(base_size = 11) +
  theme(legend.position = "bottom")
...
```{r}
# Step 1: Prepare total coral dataset
summary_total <- preds_total_avg %>%
  group_by(.draw, dataset) %>%
  summarize(total_density = sum(mean_epred), .groups = "drop") %>%
  group_by(dataset) %>%
  ...
  filter(dataset %in% c("dca17", "tt21", "tt23", "drm24")) %>%
  mutate(
    dataset = factor(dataset, levels = c("dca17", "tt21", "tt23", "drm24")),
    year = parse_number(as.character(dataset)),
    sus_group = "total"
  )

# Step 2: Combine with susceptibility summaries
combined <- bind_rows(
  summary_total %>% select(dataset, year, total_density, .lower, .upper, sus_group),
...
# Step 3: Compute linear model stats
lm_stats <- combined %>%
  group_by(sus_group) %>%
  do(broom::tidy(lm(total_density ~ year, data = .))) %>%
  filter(term == "year") %>%
  mutate(label = paste0("Slope = ", signif(estimate, 2), ", p = ", signif(p.value, 2)))
...
ggsave("outputs/Fig5_all_and_sus.png", width = 180, height = 80, units = "mm", dpi = 300)





# Add group label to total coral summary
summary_total_filtered2 <- summary_total_filtered %>%
  mutate(sus_group = factor("total", levels = c("total", "low_sus", "med_sus", "high_sus")))

# Combine with susceptibility group data
summary_combined <- bind_rows(summary_total_filtered2, summary_sus_filtered)

lm_stats_full <- summary_combined %>%
  group_by(sus_group) %>%
  do({
...
changeplot + 
  geom_text(
    data = lm_labels2,
    aes(x = x, y = Inf, label = label),
    hjust = 0, vjust = 4, size = 3, color = "blue",
    inherit.aes = FALSE
  )

ggsave(filename = "outputs/Fig5.png", width = 200, height = 80, units = "mm")
```
