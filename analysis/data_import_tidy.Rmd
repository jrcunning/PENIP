...
Remove cervicornis (zeros) from tt23 dataset?
```{r}
dstax <- dff %>%
...
newdata <- dfftaxon %>% #dfftaxon_trimmed2 %>%
...
plot_total <- ggplot(summary_total_density_by_type, aes(x = Type, y = mean)) +
  ...
  scale_y_continuous(limits = c(0, 2.9), breaks = seq(0, 2.5, 0.5), minor_breaks = seq(0, 2.75, 0.25), expand = expansion(mult = c(0.02, 0))) +
  labs(
    x = "Habitat Type",
    y = "Density (per m²)",
    title = NULL
...
ggsave(filename = "outputs/FigA4.png", width = 180, height = 220, units = "mm", dpi = 300)
...
figA6 <- raw_vs_mod + theme_minimal() +
  theme_minimal() +
  theme(
    legend.position = "bottom", 
    panel.background = element_rect(fill = "white", color = NA),
...
ggsave(filename = "outputs/FigA6.png", width = 190, height = 250, units = "mm")
