...
## Impact Zones
#### Scenario 2 + NMFS consideration areas
```{r}
...
# Step 1: Compute centroids of each polygon
zone_centroids <- zones_merged %>%
  st_cast("POLYGON") %>%
  mutate(centroid = st_centroid(geometry)) %>%
  mutate(centroid_coords = st_coordinates(centroid))

# Step 2: Get channel Y center (assumes UTM/meter CRS)
channel_y_center <- channel_poly %>%
  st_centroid() %>%
  st_transform(4326) %>%
  st_coordinates() %>%
  .[[2]]  # This is latitude in degrees


# Step 3: Classify each polygon as north/south or single
zone_centroids <- zone_centroids %>%
  mutate(
    label_group = case_when(
      name %in% c("North", "South", "East", "Nearshore") ~ "single",
      st_coordinates(centroid)[, 2] >= channel_y_center ~ "north",
      TRUE ~ "south"
    )
  )

# Step 4: Create manual label position table
zone_label_positions <- tribble(
  ~name, ~label_group, ~label_x, ~label_y, ~segment_x, ~segment_y,
  "Side Slopes", "north", -80.106, 26.096, -80.104, 26.0945,
  "Depth_10cm", "north", -80.106, 26.0975, -80.1035, 26.0947,
  "Depth_5cm", "north", -80.106, 26.099, -80.103, 26.0949,
  "Depth_1cm", "north", -80.106, 26.1005, -80.1028, 26.0957,
  "Depth_0_5cm", "north", -80.106, 26.102, -80.1026, 26.0968,
  "Depth_0_1cm", "north", -80.106, 26.1035, -80.1024, 26.098,
  "Nearshore", "single", -80.107, 26.093, -80.106, 26.0899,
  "Side Slopes", "south", -80.106, 26.088, -80.104, 26.093,
  "Depth_10cm", "south", -80.106, 26.0865, -80.1035, 26.09285,
  "Depth_5cm", "south", -80.106, 26.085, -80.103, 26.0926,
  "Depth_1cm", "south", -80.106, 26.0835, -80.1028, 26.092,
  "Depth_0_5cm", "south", -80.106, 26.082, -80.1026, 26.091,
  "Depth_0_1cm", "south", -80.106, 26.0805, -80.1024, 26.0897,
  "North", "single", -80.080, 26.097, -80.080, 26.097,
  "East", "single", -80.0805, 26.094, -80.0805, 26.094,
  "South", "single", -80.080, 26.091, -80.080, 26.091
  # Add more as needed
)

# Step 5: Join label data back to zones
zone_label_geom <- zone_centroids %>%
  left_join(zone_label_positions, by = c("name", "label_group"))

# Step 6: Plot
map_zones <- p +
  geom_segment(data = zone_label_geom,
               aes(x = segment_x, y = segment_y,
                   xend = label_x, yend = label_y),
               color = "black", linewidth = 0.2) +
  geom_text(data = zone_label_geom,
...
cowplot::plot_grid(fig1, iz_plot, ncol = 2, rel_widths = c(0.67, 0.33))
...
# 1. Create the individual ESA facet plots (no legend shown)
...
# 3. Extract the fill legend from polyplot (which includes habitat fill)
