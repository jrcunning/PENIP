...
set.seed(124)
yrep  <- posterior_predict(mod_nb3.4)
yrep <- sweep(yrep, 2, area, FUN = "/")
set.seed(124)
mu    <- posterior_epred(mod_nb3.4)  # matrix: draws x observations
...
#   distinct(dataset, taxon, class, Type, dir, site) %>%
#   mutate(transect_area_m2 = 1)
# 
# # Step 3: Get population-level predictions from the model (includes all group-level effects)
# preds <- add_epred_draws(mod_nb3.4, newdata = newdata, re_formula = NULL)
# 
...
# summary_equal_weighted_orig <- draws_equal_weighted %>%
#   group_by(taxon, class, Type) %>%
#   mean_qi(n_m2, .width = 0.95)
```

### Final density estimations -- modified approach

```{r}
# # Same newdata
# preds_hier <- add_epred_draws(
#   mod_nb3.4,
#   newdata = newdata,
#   re_formula = NA  # includes all dataset REs (marginalized site & dir automatically)
# )
# 
# # Directly summarize posterior draws (no equal weighting applied)
# summary_hier <- preds_hier %>%
#   group_by(taxon, class, Type) %>%
...
ggsave(filename = "outputs/FigA3.png", width = 180, height = 220, units = "mm", dpi = 300)
