...
mod_nb3 <- brm(
  ...
  prior = c(
    ...
    prior(normal(1.5, 2), class = "b", dpar = "zi"),      
    # Matches empirical shape estimates
    prior(normal(log(0.5), 1), class = "b", dpar = "shape") 
  ),
...
ppreds <- add_predicted_draws(mod_nb3, newdata = dfftaxon, ndraws = 600)

# Step 3: Summarize predicted statistics by taxon_class and draw
pp_stats <- ppreds %>%
  group_by(taxon_class, .draw) %>%
  summarize(
