...
taxon_order <- tile_data %>%
  ...
  summarize(total_n = sum(tot), 
            total_tiles = sum(n_tiles), .groups = "drop") %>%
  arrange(total_n) %>%
  mutate(taxon_row_start = cumsum(lag(total_tiles %/% tiles_per_row + 1, default = 0)))
...
tile_expanded <- tile_data %>%
  ...
  mutate(
    ...
    row = -(taxon_row_start + (tile_id - 1) %/% tiles_per_row)
  ) %>%
  ungroup() %>%
  mutate(taxon = factor(taxon, levels = taxon_order$taxon))
...
row_labels <- tile_expanded %>%
  group_by(taxon) %>%
  summarize(label_y = min(row), 
            label_x_end = max(col)[row == min(row)], 
            .groups = "drop")
row_labels <- tile_expanded %>%
  group_by(taxon) %>%
  filter(row == min(row)) %>%     # only keep the top row
  summarize(
    label_y = unique(row),        # top row
    label_x_end = max(col),       # rightmost tile in that row
    .groups = "drop"
  )
...
waffle <- ggplot(tile_expanded, aes(x = col, y = row, fill = taxon)) +
  ...
  geom_point(
    data = filter(tile_expanded, class == "<4cm"),
    aes(x = col, y = row),
    shape = 16, color = "white", size = 0.3, inherit.aes = FALSE
  ) +
  ...
  geom_text(
    data = row_labels,
    aes(x = -2, y = label_y, label = taxon),
    hjust = 1, size = 2, inherit.aes = FALSE
  ) +
  ...
  geom_text(
    data = row_labels,
    aes(x = label_x_end, y = label_y, label = total_fmt),
    hjust = -0.1, size = 2, inherit.aes = FALSE
  ) +
...
waffle
ggsave(filename = "outputs/Fig4.png", height = 250, width = 145, units = "mm", dpi = 300)
```
