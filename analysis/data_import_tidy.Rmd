...
adults0 <- read_csv("data/DRM_broward_corals.csv")
adults_long <- adults0 %>%
  ...
  select(team, site, transect_num, taxon = species, max_width_cm) %>%
...
drmjuv <- read_csv("data/DRM_broward_juveniles.csv") %>%
  mutate(site = str_remove(site, "^AA")) %>%
...
drmjuv_long <- drmjuv %>%
  pivot_longer(4:7, names_to = "taxon", values_to = "n") %>%
  mutate(max_width_cm = "<4") %>%
  uncount(n)

# Full juvenile counts from Transects 1 and 2 (bonus data)
t1t2bonus <- read_csv("data/T1_T2_bonus_data.csv") %>%
  janitor::clean_names() %>%
  mutate(site = replace_na(site, "NA")) %>%    # Because one site is called "NA"
  mutate(team = "Shedd Aquarium", transect_num = parse_number(transect))
t1t2juv <- t1t2bonus %>%
  select(team, site, transect_num, starts_with("small")) %>%
  rename_with(~ toupper(gsub("^small_", "", .x)), starts_with("small_"))
t1t2juv_long <- t1t2juv %>%
  pivot_longer(4:11, names_to = "taxon", values_to = "n") %>%
  mutate(max_width_cm = "<4") %>%
  uncount(n)

# Replace site names in this t1t2 bonus data with the correct DRM site ID
penipsites <- readxl::read_xlsx("data/site_metadata.xlsx") %>%
  janitor::clean_names()
...
# Combine
drm_long <- bind_rows(adults_long, drmjuv_long, t1t2juv_long_updated)
...
search_grid <- drm_classed %>%
  distinct(site, transect_num, team) %>%
  mutate(
