...
drm24_counts_ag %>% filter(site == "1132", taxon == "MMEA")
# why only 3 transects and not 4?
# issues with how these are being counted and assigned explicit zeros or missing values... reevaluate!
```
...
```{r total_density, include = F, eval = F}
...
```{r total_density_ns_diffs, include = F, eval = F}
# Any N-S differences in total coral abundance within habitat Types?
...
There is a significant difference on the Outer Reef, where there are more corals to the South than to the North -- this was significant in the 2017 DCA, 2021 TT, and 2023 TT surveys.

### Survey differences?
```{r total_density_survey_diffs, include = F, eval = F}
...
# 4. Filter significant differences (exclude ESA-only surveys)
summary_total_contrasts %>%
  filter(!str_detect(dataset1, "_esa") & !str_detect(dataset2, "_esa")) %>%
  mutate(p_adj = p.adjust(p_two_sided, method = "bonferroni")) %>%
  filter(p_adj < 0.01) %>%
  arrange(Type, dir) %>%
  knitr::kable()
...
test <- dfftaxon %>%
  filter(dataset == "nsu11_esa", taxon == "ACER")
```
...
mod_nb2 <- brm(
  bf(n ~ dataset * taxon * class * Type + 
       (1 | dataset:Type:taxon:class:dir) +
       (1 | dataset/site) + 
       offset(log(transect_area_m2)),
     zi ~ taxon * Type),
  family = zero_inflated_negbinomial(),
...
```{r}
loo_dirFE <- loo(mod_nb)
loo_dirRE <- loo(mod_nb2)

loo_compare(loo_dirFE, loo_dirRE)
...
draws_collapsed <- posterior_draws2 %>%
...
group_definitions <- tibble(
  taxon = unique(posterior_draws2$taxon),
...
draws_grouped <- posterior_draws2 %>%
...
posterior_weighted <- posterior_draws2 %>%
  mutate(dataset = as.character(dataset)) %>%
...
p2 + scale_y_log10() + geom_hline(yintercept = c(1e6, 1e7, 1e8))
