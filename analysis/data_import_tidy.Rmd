...
df_completed %>%
  filter(dataset == "nsu11_esa", site == "1")
```
...
```{r}
# Convergence check
summary(mod_nb3.4)  # confirm Rhat ≈ 1.00, no divergences
...
ppc_densities <- ppc_counts_long %>%
  left_join(
    dfftaxon_trimmed2 %>% mutate(row = dplyr::row_number()),
    by = "row"
  ) %>%
...
area <- mod_nb3.4$data$transect_area_m2

y <- mod_nb3.4$data$n / area
set.seed(124)
yrep  <- posterior_predict(mod_nb3.4, ndraws = 1000)
yrep <- sweep(yrep, 2, area, FUN = "/")
set.seed(124)
mu    <- posterior_epred(mod_nb3.4, ndraws = 1000)  # matrix: draws x observations
...
ppreds <- add_predicted_draws(mod_nb3.4, newdata = dfftaxon_trimmed2, ndraws = 200) %>%
...
plot_total <- ggplot(summary_total_density_by_type, aes(x = Type, y = mean)) +
  ...
  scale_y_continuous(limits = c(0, 2.6), breaks = seq(0, 2.5, 0.5), minor_breaks = seq(0, 2.6, 0.25), expand = expansion(mult = c(0.02, 0))) +
  labs(
    x = "Habitat Type",
    y = "Density (per m²)",
    title = NULL
...
combined_plot
ggsave(filename = "outputs/Fig3new.png", width = 180, height = 80, units = "mm", dpi = 300)
...
ggsave(filename = "outputs/Fig4new.png", width = 180, height = 130, units = "mm", dpi = 600)
...
ggsave(filename = "outputs/FigA3new.png", width = 180, height = 220, units = "mm", dpi = 300)
...
ggplot(all, aes(x = Type)) +
  geom_point(aes(y = raw_density_mean, shape = dataset, group = dataset), size = 2,
             position = position_dodge(width = 0.6), color = "blue", alpha = 0.5) +
  geom_errorbar(aes(ymin = raw_density_lower, ymax = raw_density_upper, group = dataset), 
                position = position_dodge(width = 0.6), color = "blue", width = 0, alpha = 0.5) +
  geom_point(aes(y = n_m2), pch = 1, size = 3,
             color = "black", alpha = 0.5) +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), 
                color = "black", width = 0, alpha = 0.5) +
  facet_wrap(~interaction(taxon,class), scales = "free") +
  scale_y_continuous(limits = c(0, NA)) +
  coord_cartesian(clip = "off")
```
